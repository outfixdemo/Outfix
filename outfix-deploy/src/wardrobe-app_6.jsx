...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  const PostCard = ({feed})=>{
    const gradients = [
      "linear-gradient(135deg,#1A2A1A,#2A3A20)",
      "linear-gradient(135deg,#1A1A2E,#2A2040)",
      "linear-gradient(135deg,#2A1A10,#3A2A18)",
      "linear-gradient(135deg,#1A1A1A,#2A2830)",
    ];
    const grad = gradients[(feed.id-1)%gradients.length];
    const accentColors = ["#80C880","#9080D8","#C4A882","#60A8C8"];
    const accent = accentColors[(feed.id-1)%accentColors.length];
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:16,border:`1px solid ${BR}`}}>
        {/* Image area — gradient with outfit name */}
        <div style={{width:"100%",position:"relative",background:_1a}}>
          <div style={{width:"100%",paddingTop:"80%",position:"relative",overflow:"hidden"}}>
            <div style={{..._abs0,background:grad,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{textAlign:"center",padding:"0 24px"}}>
                <div style={{fontSize:40,marginBottom:12,opacity:0.6}}>👗</div>
                <div style={{...sr(16,300,accent,{letterSpacing:2,textTransform:"uppercase"})}}>{feed.caption||"Community Look"}</div>
              </div>
            </div>
          </div>
...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
  ...
  const [voiceDesc,setVoiceDesc]=useState("");  const [voiceDesc,setVoiceDesc]=useState("");
  ...
  const closeAdd=()=>{setShowAdd(false);setScanned(null);setUrl("");setAddMode(null);setVoiceDesc("");setPhotoPreview(null);setDetectedItems([]);setSelectedDetected({});setDescribeResults([]);setDescribeLoading(false);setMName("");setMBrand("");setMPrice("");setMCat("Tops");setMDate("");setMCondition("Good");setMColor("#C4A882");setShowBrandList(false);};
                  ...
                  <div style={{position:"relative",width:72,height:72,flexShrink:0}}>
                    ...
                    <div style={{width:72,height:72,borderRadius:14,background:`linear-gradient(135deg,${scanned.color}22,${scanned.color}55)`,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",border:_2a}}>
                      ...
                      {(!scanned.useUserPhoto && scanned.stockImage)
                        ? <img src={scanned.stockImage} style={{width:"100%",height:"100%",objectFit:"contain",padding:4,boxSizing:"border-box",mixBlendMode:"multiply"}} alt={scanned.name}/>
                        : photoPreview
...
function CropModal({src, onCancel, onSave}){
  ...
  const applyCrop=async()=>{
    ...
    // Keep a reference to the base64 version for CORS fallback
    const base64Src = imgSrc;
    ...
    img.onload=()=>{
      try{
        ...
        // Canvas tainted — retry with the blob-fetched base64 which avoids CORS
        ...
        img2.onload=()=>{
          try{
            ...
            const sw=Math.round(cropSize*scale);
            const sh=Math.round(cropSize*scale);
            ctx.drawImage(img2,sx,sy,sw,sh,0,0,outputSize,outputSize);
            onSave(canvas.toDataURL("image/jpeg",0.9));
          }catch(e2){ onSave(base64Src); } // last resort: save the fetched base64 as-is
        };
        img2.onerror=()=>onSave(base64Src);
        img2.src=base64Src;
      }
    };
    img.onerror=()=>{
      // Try loading from base64 directly
      const img2=new Image();
      img2.onload=()=>{
        try{
          const scale=imgNatural.w/displaySize.w;
          ctx.drawImage(img2,Math.round(cropX*scale),Math.round(cropY*scale),Math.round(cropSize*scale),Math.round(cropSize*scale),0,0,outputSize,outputSize);
          onSave(canvas.toDataURL("image/jpeg",0.9));
        }catch(e){ onSave(base64Src); }
      };
      img2.onerror=()=>onSave(base64Src);
      img2.src=base64Src;
    };
    ...
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {showBuilder&&(
        <div onClick={()=>setShowBuilder(false)} style={{..._fix,background:"#000000AA",zIndex:60,display:"flex",alignItems:"flex-start"}}>
          ...
          {builder.length>0&&(
            <div style={{display:"flex",gap:8}}>
              <button onClick={save} style={{flex:1,padding:"12px",borderRadius:14,background:`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(9,600,BK,{letterSpacing:1.5}),cursor:_p}}>SAVE LOOK</button>
            </div>
          )}
        </div>
        </div>
...
// ── CREATE LISTING PAGE ───────────────────────────────────────────────────────
function DuplicatesSection({items,showToast,setTab}){
    ...
    <div>
      {duplicateGroups.map(group=>(
        <div key={group.id} style={{background:CD,borderRadius:16,padding:"16px",marginBottom:12,border:`1px solid ${BR}`}}>
          ...
          {group.similarity>0&&(
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{if(setTab) setTab("market"); showToast("Go to Market to list this item \u2746");}} style={{flex:1,padding:"8px",borderRadius:11,background:_1a,border:_2a,...ss(8,500,DM,{letterSpacing:0.8}),cursor:_p}}>LIST THE WEAKER ONE</button>
...
function StatsTab({items, outfits, showToast, logWear, setTab}){
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
          ...
          <div style={{background:CD,borderRadius:18,padding:"18px",border:"1px solid #2A1818"}}>
            ...
            {neglected.map(item=>(
              <div key={item.id} style={{..._row,gap:12,marginBottom:10}}>
                ...
                <button onClick={()=>showToast("Go to Market tab to list this item \u2746")} style={{padding:"5px 12px",borderRadius:20,background:"#2A1A1A",border:"1px solid #4A2A2A",cursor:_p,...ss(9,600,"#C4A0A0",{letterSpacing:1})}}>SELL</button>
              </div>
            ))}
          </div>
        </>
      ...
      {/* ── WARDROBE HEALTH ── */}
      {section==="dupes"&&(
        <DuplicatesSection items={items} showToast={showToast} setTab={setTab}/>
      )}
    ...
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {section==="privacy"&&(
        <div>
          ...
          <button onClick={async()=>{
            if(!window.confirm("Permanently delete your account? This cannot be undone — all your items, outfits and data will be deleted.")) return;
            try {
              // Delete all user data from Supabase
              const token = typeof onSignOut === 'function' ? null : null;
              showToast("Deleting account… \u2746");
              // Sign out first, which clears local session
              if(onSignOut) onSignOut();
            } catch(e) {
              showToast("Please contact support to delete your account \u2746");
            }
          }} style={{width:"100%",marginTop:10,padding:"12px",borderRadius:14,background:"#1A0A0A",border:"1px solid #3A1A1A",...ss(10,600,"#C46060",{letterSpacing:1}),cursor:_p}}>
            DELETE ACCOUNT
          </button>
        </div>
      )}
...
// ── VAULT ────────────────────────────────────────────────────────────────────
function VaultTab({items,outfits,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,currentPlan,setShowPricing,logWear,events,setEvents,session,setTab}){
    ...
    <div className="fu" style={{padding:"0 0 24px"}}>
      ...
      {section==="stats"&&(
        <Gate feature="Wardrobe Stats">
          <StatsTab items={items} outfits={outfits} showToast={showToast} logWear={logWear} setTab={setTab}/>
        </Gate>
      )}
      {section==="shoppers"&&(
