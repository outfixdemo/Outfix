...
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit,styleProfile={},saveStyleProfile}){
  ...
  const suggestWithAI=async()=>{
    ...
    if(styleProfile.likedCombos?.length > 0){
      const recent = styleProfile.likedCombos.slice(-5).map(c=>c.names?.join(" + ")).filter(Boolean);
      if(recent.length) profileParts.push(`Previously liked: ${recent.join(" | ")}`);
    }
    if(styleProfile.dislikedCombos?.length > 0){
      ...
      if(recent.length) profileParts.push(`Previously disliked (avoid similar): ${recent.join(" | ")}`);
    }
      ...
      {aiVibe && !aiLoading && (
        <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center",justifyContent:"center"}}>
          ...
          <button onClick={()=>{
            ...
            if(saveStyleProfile) saveStyleProfile({likedCombos:[...(styleProfile.likedCombos||[]),{names,vibe:aiVibe,ts:Date.now()}].slice(-20)});
          ...
          <button onClick={()=>{
            const combo = [top,isDress?null:bottom,shoe,showOuterwear?outer:null,showAccessories?accessory:null].filter(Boolean);
            const names = combo.map(i=>i.name);
            if(saveStyleProfile) saveStyleProfile({dislikedCombos:[...(styleProfile.dislikedCombos||[]),{names,vibe:aiVibe,ts:Date.now()}].slice(-20)});
