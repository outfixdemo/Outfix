...
Respond ONLY with JSON: {"days":[{"day":1,"date":"${tripData.startDate}","label":"Arrival & Check-in","activity":"Travel","emoji":"✈️"},...]}`
  ...
  const handleVacSwipeEnd=e=>{
    ...
    if(Math.abs(dx)>50){
      const newIdx=dx<0
        ? Math.min(vacIdx+1,trips.length-1)
        : Math.max(vacIdx-1,0);
      if(newIdx!==vacIdx){ setVacIdx(newIdx); setVacation(trips[newIdx]); }
    }
    setSwipeStartX(null);
  };
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {trips.length>1&&(
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:6}}>
            {trips.map((_,i)=>(
              <div key={i} onClick={()=>{setVacIdx(i);setVacation(trips[i]);}}
      ...
      <div onTouchStart={handleVacSwipeStart} onTouchEnd={handleVacSwipeEnd}
        style={{background:"linear-gradient(135deg,#0F1A2E,#162236)",borderRadius:20,padding:"20px",border:"1px solid #2A3A5A",marginBottom:16,position:"relative",overflow:"hidden"}}>
