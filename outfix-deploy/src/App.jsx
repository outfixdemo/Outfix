  ...
  return new Promise(resolve=>{
    ...
    img.onload=()=>{
      ...
      for(let i=0;i<w*h;i++){
        ...
        if(whiteness>200&&a<230){
          // Ramp: fully white + very transparent → erase completely
          // Slightly off-white or near-opaque → keep but reduce
          const whiteScore=(whiteness-200)/55; // 0→1 as whiteness goes 200→255
          const alphaScore=1-(a/230);          // 0→1 as alpha goes 230→0
          const eraseStrength=whiteScore*alphaScore;
          d[i*4+3]=Math.round(a*(1-eraseStrength));
        }
      }
