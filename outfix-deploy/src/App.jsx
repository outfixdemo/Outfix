        ...
        onUpdate={(updated)=>{
          ...
          if(session?.access_token){
            ...
            const persist = async () => {
              ...
              // Sync feed posts — update item_image on any feed_events for this item
              if(sourceImageUrl){
                const userId2 = session.user?.id ||
                  (() => { try { return JSON.parse(atob(session.access_token.split(".")[1])).sub; } catch(e){ return null; } })();
                // Update events that have item_id set (new events)
                fetch(`${SB_URL}/rest/v1/feed_events?item_id=eq.${updated.id}`,{
                  method:"PATCH",
                  headers:{...sbHeaders(session.access_token),"Prefer":"return=minimal"},
                  body:JSON.stringify({item_image:sourceImageUrl}),
                }).catch(()=>{});
                // Also update older events by user_id + item_name (events before item_id column existed)
                if(userId2 && updated.name){
                  fetch(`${SB_URL}/rest/v1/feed_events?user_id=eq.${userId2}&item_name=eq.${encodeURIComponent(updated.name)}&item_id=is.null`,{
                    method:"PATCH",
                    headers:{...sbHeaders(session.access_token),"Prefer":"return=minimal"},
                    body:JSON.stringify({item_image:sourceImageUrl}),
                  }).catch(()=>{});
                }
                // Update liveEvents in memory so feed reflects change immediately
                setLiveEvents(prev=>prev.map(e=>{
                  const matchById = String(e.item_id)===String(updated.id);
                  const matchByName = !e.item_id && e.user_id===userId2 && e.item_name===updated.name;
                  return (matchById||matchByName) ? {...e,item_image:sourceImageUrl} : e;
                }));
              }
            };
            persist();
