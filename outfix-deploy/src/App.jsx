...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  const LiveEventCard = ({event}) => {
    ...
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:16,border:`1px solid ${G}22`}}>
        {/* Hero */}
        <div style={{width:"100%",position:"relative"}}>
          <div style={{width:"100%",paddingTop:isWore?"56%":"70%",position:"relative",overflow:"hidden"}}>
            {!isWore && event.item_image ? (
              <img src={event.item_image} style={{..._abs0,width:"100%",height:"100%",objectFit:"contain",padding:24,boxSizing:"border-box",background:"#0F0F0F"}} alt={event.item_name}/>
            ) : (
              <div style={{..._abs0,background:`linear-gradient(135deg,${G}12,${G}28)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8}}>
                {isWore ? (
                  <div style={{display:"flex",gap:12,fontSize:40}}>{emojis.length ? emojis.map((e,i)=><span key={i}>{e}</span>) : ["👗","👟","✨"].map((e,i)=><span key={i}>{e}</span>)}</div>
                ) : (
                  <div style={{fontSize:52}}>{event.item_emoji||"👗"}</div>
                )}
              </div>
            )}
          </div>
        ...
        <div style={{width:"100%",position:"relative",background:_1a}}>
          <div style={{width:"100%",paddingTop:"80%",position:"relative",overflow:"hidden"}}>
            <div style={{..._abs0,background:`linear-gradient(135deg,${accent}18,${accent}38)`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{display:"flex",gap:16,fontSize:44}}>{emojis.map((e,i)=><span key={i}>{e}</span>)}</div>
            </div>
          </div>
  ...
  return(
    <div ref={feedRef} className="fu" style={{padding:"0 16px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
