...
// ── HOME ─────────────────────────────────────────────────────────────────────
// ── STORY VIEWER ─────────────────────────────────────────────────────────────
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session}){
      ...
      <div style={{marginBottom:12}}>
        ...
        {showFilterMenu&&(
          <div style={{background:"#0F0F0F",borderRadius:14,border:_2a,padding:"16px",marginBottom:12}}>
            {/* Category */}
            <div style={ss(8,600,DM,{letterSpacing:1.5,marginBottom:8})}>CATEGORY</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
              {cats.map(c=>(
                <button key={c} onClick={()=>{setFilterCat(c);}} style={{
                  padding:"5px 12px",borderRadius:20,whiteSpace:"nowrap",cursor:_p,
                  background:filterCat===c?G:"#1A1A1A",border:filterCat===c?"none":"1px solid #2A2A2A",
                  ...ss(9,filterCat===c?600:400,filterCat===c?BK:DM,{letterSpacing:0.8}),
                }}>{c}</button>
              ))}
            </div>
            ...
            {showSortExpanded&&(
              <div onClick={()=>setShowSortExpanded(false)} style={{..._fix,inset:0,background:"#000000AA",zIndex:200,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 8px 8px"}}>
...
function SwipeRow({label,arr,idx,setIdx,emoji,isLocked,onLockToggle,onMarkUnavailable,onCycleEnd}){
  ...
  return(
    <div style={{position:"relative",height:110,marginBottom:8,touchAction:"pan-y",maxWidth:380,margin:"0 auto 8px"}}>
      {/* Behind card peek */}
      {behindItem&&arr.length>1&&(dragging||(flying!==0))&&(
        <div style={{..._abs0,borderRadius:18,overflow:"hidden",background:"#111",border:`1px solid #2A2A2A`,transform:`scale(${0.94+dragPct*0.06})`,transition:"transform 0.1s",display:"flex",alignItems:"center"}}>
          <div style={{width:90,height:110,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
            {behindItem.sourceImage
              ? <img src={behindItem.sourceImage} style={{width:"100%",height:"100%",objectFit:"contain",mixBlendMode:"multiply",padding:8,boxSizing:"border-box"}} alt={behindItem.name}/>
              : <ItemIllustration item={behindItem} size={70}/>
            }
          </div>
          <div style={{flex:1,padding:"0 14px 0 4px"}}>
            <div style={sr(14,500,"#E8E0D4",{opacity:0.4,lineHeight:1.3})}>{behindItem.name}</div>
          </div>
        </div>
      )}
      ...
      <div key={idx} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onTransitionEnd={flying!==0?onFlyEnd:undefined}
        style={{
          ...
          background:"#111",
          border:isLocked?`2px solid ${G}`:`1px solid #2A2A2A`,
        ...
        {/* Image — left side */}
        <div style={{width:90,height:110,flexShrink:0,background:"#111",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",borderRight:"1px solid #1E1E1E"}}>
          {item.sourceImage
            ? <img src={item.sourceImage} style={{width:"100%",height:"100%",objectFit:"contain",mixBlendMode:"multiply",padding:8,boxSizing:"border-box"}} alt={item.name}/>
            : <ItemIllustration item={item} size={72}/>
          }
        </div>

        {/* Text — right side */}
        <div style={{flex:1,padding:"0 14px 0 14px",minWidth:0}}>
          <div style={ss(7,600,DM,{letterSpacing:1.5,marginBottom:4})}>{label.toUpperCase()}</div>
          <div style={sr(15,500,"#F0EBE3",{lineHeight:1.3,marginBottom:3})}>{item.name}</div>
          <div style={ss(9,400,DM)}>{item.brand}</div>
          {isLocked&&(
            <div onClick={e=>{e.stopPropagation();onLockToggle();}} style={{marginTop:6,display:"inline-flex",alignItems:"center",gap:4,background:`${G}22`,border:`1px solid ${G}44`,borderRadius:8,padding:"2px 8px",cursor:_p,...ss(8,600,G,{letterSpacing:0.5})}}>🔒 LOCKED</div>
          )}
          {item.forSale&&!isLocked&&(
            <div style={{marginTop:6,display:"inline-block",background:G,borderRadius:8,padding:"2px 8px",...ss(7,700,BK,{letterSpacing:1})}}>FOR SALE</div>
          )}
        </div>

        {/* Swipe direction hints */}
        {dragX<-20&&<div style={{position:"absolute",top:12,right:12,border:"2px solid #E08080",borderRadius:10,padding:"3px 10px",...ss(10,700,"#E08080",{letterSpacing:2}),opacity:Math.min(1,(-dragX-20)/60),transform:"rotate(-4deg)"}}>NEXT</div>}
        {dragX>20&&<div style={{position:"absolute",top:12,left:100,border:"2px solid #80C880",borderRadius:10,padding:"3px 10px",...ss(10,700,"#80C880",{letterSpacing:2}),opacity:Math.min(1,(dragX-20)/60),transform:"rotate(4deg)"}}>PREV</div>}
...
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit}){
  ...
  const saveCurrentAsOutfit=async()=>{
    const combo=[top,bottom,shoe,showOuterwear?outer:null,showAccessories?accessory:null].filter(Boolean);
...
// ── USER PROFILE PAGE ─────────────────────────────────────────────────────────
function UserProfilePage({ handle, userId, username, onClose, showToast, session }) {
            ...
            {(profile.allItems||[]).map((item,i)=>(
              <div key={i} style={{background:"#111",borderRadius:14,overflow:"hidden",border:"1px solid #1E1E1E"}}>
                ...
                <div style={{padding:"8px 10px 10px"}}>
                  ...
                  <div style={sr(12,500,G,{marginTop:3})}>${item.price||0}</div>
                </div>
              </div>
            ))}
          </div>
...
function WishlistAddModal({onClose, onAddToWishlist}){
  ...
  const fetchUrl=async()=>{
    ...
    try{
      ...
      const json=JSON.parse(raw.replace(/```json|```/g,"").trim());
      onAddToWishlist({id:Date.now(),name:json.name,brand:json.brand,price:productRes.price||json.price||0,emoji:json.emoji||"👗",gap:json.note||"Saved from URL",inMarket:false,sourceImage:productRes.image||null,color:json.color||"#C4A882"});
      onClose();
...
function VacationPlanner({items,outfits,showToast,onBack}){
  ...
  // Auto-fetch weather when planner loads
              ...
              <input
                ...
                onChange={e=>{
                  ...
                  destTimer.current=setTimeout(async()=>{
                    try{
                      const res=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(v)}&format=json&limit=6&featuretype=city,country,state&addressdetails=1`,{headers:{"Accept-Language":"en"}});
                      const data=await res.json();
                      const seen=new Set();
              ...
              <div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:50,background:"#1A1A1A",borderRadius:"0 0 12px 12px",border:`1px solid ${G}44`,borderTop:"none",overflow:"hidden",marginTop:-2}}>
                {destSuggestions.map((s,i)=>(
                  <div key={i} onMouseDown={()=>{setForm(p=>({...p,destination:s}));setDestSuggestions([]);setShowDestList(false);}}
                    style={{padding:"11px 14px",cursor:_p,borderTop:i>0?`1px solid #2A2A2A`:"none",display:"flex",alignItems:"center",gap:10,...ss(13,400,MD)}}
                    className="ch">
...
export default function App(){
  ...
  const saveProfile = async (updates) => {
    ...
    try{
      ...
      if(!r.ok) console.error("saveProfile error:", r.status, await r.text());
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",overflow:"hidden",transition:"background 0.3s,color 0.3s"}}>
      ...
      <div className="sc" style={{height:"calc(100vh - 130px)",paddingBottom:80}}>
        {tab==="home"     && <HomeTab items={items} outfits={outfits} showToast={showToast} setTab={setTab} setWishlist={setWishlist} addToWishlist={addToWishlist} removeFromWishlist={removeFromWishlist} setItems={setItems} session={session}/>}
