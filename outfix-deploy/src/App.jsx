...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  // Attach pull-to-refresh directly to the main scroll container
  useEffect(()=>{
    ...
    const onMove = (e) => {
      ...
      if(el.scrollTop > 0){ ptrState.current.tracking=false; setPullProgress(0); return; }
    ...
    const onEnd = () => {
      ...
      setPullProgress(prev => {
        if(prev >= 1 && !ptrState.current.refreshing){
          ...
          setRefreshing(true);
          // loadLiveFeed is defined below — call via ref
  ...
  }, []);  // mount once — uses refs so no stale closure
...
function UserProfilePage({ handle, userId, username, onClose, showToast, session, onAddToCloset }) {
  ...
  return (
    <div style={{..._fix,background:BK,zIndex:400,maxWidth:430,margin:"0 auto",display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      ...
      {/* ── ITEM DETAIL SHEET ── */}
      {selectedProfileItem&&(
        <div onClick={()=>setSelectedProfileItem(null)} style={{..._fix,background:"#000C",zIndex:500,display:"flex",alignItems:"flex-start"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"0 0 24px 24px",width:"100%",maxWidth:430,margin:"0 auto",maxHeight:"88vh",overflowY:"auto",border:`1px solid ${G}22`}}>
            {/* Handle */}
            <div style={{width:36,height:4,borderRadius:2,background:"#333",margin:"12px auto 8px"}}/>
            {/* Image */}
