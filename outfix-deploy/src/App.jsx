...
// Removes semi-transparent white/near-white residue left behind on clear items (glasses lenses, etc.)
// Targets pixels that are near-white in color AND partially transparent — the "fog" signature.
  ...
  return new Promise(resolve=>{
    ...
    img.onload=()=>{
      ...
      for(let i=0;i<w*h;i++){
        ...
        // Near-white pixel: all channels high, not fully opaque
        // Soft threshold: the whiter + more transparent, the more we push alpha to 0
        const brightness=(r+g+b)/3;
        const whiteness=Math.min(r,g,b); // high = neutral white (not colored)
        // Only target genuinely foggy pixels: very white AND very transparent
        // Shirt fabric even at edges is typically alpha>160 — don't touch those
        // Real bg-spill fog is alpha<80 with near-neutral white color
        if(whiteness>210&&a<80){
          // Hard erase: these are unambiguous background residue
          d[i*4+3]=0;
        } else if(whiteness>230&&a<40){
          // Extra pass for extremely faint near-white wisps
          d[i*4+3]=0;
        }
      }
...
function CropModal({src, onCancel, onSave}){
  ...
  const applyWithBgRemoval=async()=>{
    ...
    try{
      ...
      const cleanBlob=await cleanIsolatedPixels(resultBlob);
      const finalBlob=await removeWhiteSpill(cleanBlob);
      const reader=new FileReader();
