...
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit}){
  ...
  const wearToday=()=>{
    ...
    // Check if a "Mix & Match" outfit for today already exists, else create one
    setOutfits(prev=>{
      const existing = prev.find(o=>o.name==="Mix & Match" && (o.wornHistory||[]).includes(key));
      if(existing) return prev;
      const todayOutfit = prev.find(o=>o.name==="Mix & Match");
      if(todayOutfit){
        const newHistory=[key,...(todayOutfit.wornHistory||[])];
        if(session?.access_token) sb.update("outfits",session.access_token,todayOutfit.id,{worn_history:newHistory,item_ids:combo.map(i=>i.id)}).catch(()=>{});
        return prev.map(o=>o.id===todayOutfit.id?{...o,items:combo.map(i=>i.id),wornHistory:newHistory}:o);
      }
      const newOutfit={id:Date.now(),name:"Mix & Match",items:combo.map(i=>i.id),occasion:"Casual",season:"All Year",wornHistory:[key]};
      if(session?.access_token) sb.insert("outfits",session.access_token,{name:"Mix & Match",item_ids:combo.map(i=>i.id),occasion:"Casual",season:"All Year",worn_history:[key],user_id:session.user?.id}).catch(()=>{});
      return [...prev,newOutfit];
    });
    // Increment wear count on each item
    setItems(prev=>prev.map(i=>combo.find(c=>c.id===i.id)?{...i,wearCount:i.wearCount+1,lastWorn:displayDate}:i));
    combo.forEach(item=>updateWearInDB(item.id, item.wearCount+1));
