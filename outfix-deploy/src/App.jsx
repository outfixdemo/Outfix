...
const sb = {
  ...
  loadSession() {
    try { return JSON.parse(localStorage.getItem("outfix_session")); } catch(e) { return null; }
  },
  clearSession() {
...
// ── CLOSET ───────────────────────────────────────────────────────────────────
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist}){
  ...
  const confirmAdd=()=>{
    setItems(prev=>[...prev,{...scanned,id:Date.now(),sourceImage:photoPreview||undefined}]);
    closeAdd();
  ...
  const confirmAddMulti=()=>{
    ...
    const now=Date.now();
    setItems(prev=>[...prev,...toAdd.map((item,idx)=>({...item,id:now+idx,wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",forSale:false,sourceImage:photoPreview||undefined}))]);
    closeAdd();
    showToast(`${toAdd.length} item${toAdd.length>1?"s":""} added to your closet \u2746`);
  };

  const closeAdd=()=>{setShowAdd(false);setScanned(null);setUrl("");setAddMode(null);setVoiceStage("idle");setTranscript("");setVoiceDesc("");setPhotoPreview(null);setDetectedItems([]);setSelectedDetected({});};
            ...
            <div style={{..._btwnS,marginBottom:4}}>
              <div>
                ...
                <div style={ss(9,400,DM,{letterSpacing:1,marginTop:2})}>
                  {addMode==="describe" ? "SPEAK OR TYPE · AI PARSES THE DETAILS" : "PHOTO UPLOAD  |  PASTE URL  |  DESCRIBE"}
                </div>
              </div>
              {addMode&&(
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button className="sb" onClick={async()=>{
                      ...
                      try{
                        ...
                        const json=JSON.parse(raw.replace(/```json|```/g,"").trim());
                        setScanning(false);
                        setScanned({...json,wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",forSale:false});
                      }catch(e){
                        setScanning(false);
              ...
              <div style={{marginTop:20}}>
                ...
                {voiceStage==="idle" && (
                  <>
                    <div style={{textAlign:"center",marginBottom:24}}>
                      <div style={{...ss(10,400,DM,{letterSpacing:1,marginBottom:6})}}>TAP THE MIC TO START</div>
                      <div style={ss(9,400,"#4A3A58",{lineHeight:1.6})}>e.g. "Navy blazer from Zara, double-breasted, size small, around $120"</div>
                    </div>
                    ...
                    <div style={{display:"flex",justifyContent:"center",marginBottom:20}}>
                      <button onClick={startRecording} style={{
                        ...
                        background:"linear-gradient(135deg,#2A2040,#1A1428)",
                        border:`2px solid #3A2A58`,
                        display:"flex",alignItems:"center",justifyContent:"center",
                        fontSize:32,cursor:_p,boxShadow:`0 0 0 8px #2A204030`,
                      }}>🎙️</button>
                    </div>

                    {/* Divider */}
                    <div style={{..._row,gap:12,marginBottom:16}}>
                      <div style={{flex:1,height:1,background:"#2A2A2A"}}/>
                      <div style={ss(8,400,DM,{letterSpacing:1})}>OR TYPE INSTEAD</div>
                      <div style={{flex:1,height:1,background:"#2A2A2A"}}/>
                    </div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe your item… e.g. cream silk blouse from Equipment, long sleeves, size XS, bought for $180"
                      rows={3}
                      style={{width:"100%",background:_1a,border:_2a,borderRadius:14,padding:"12px 14px",...ss(11,400,MD),color:"#C0B8B0",resize:"none",boxSizing:"border-box",lineHeight:1.6}}
                    />
                    {voiceDesc.trim() && (
                      <button onClick={()=>{setVoiceStage("parsing");setTimeout(()=>{setVoiceStage("done");setScanned({name:"Silk Ivory Blouse",brand:"Equipment",category:"Tops",color:"#F5F0E8",price:180,tags:["elegant","office"],emoji:"👚",wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",condition:"Like New",forSale:false});},2200);}}
                        style={{width:"100%",marginTop:10,padding:"12px",borderRadius:12,background:"linear-gradient(135deg,#2A2040,#1A1428)",border:"1px solid #3A2A58",...ss(10,600,"#C0B0D8",{letterSpacing:1}),cursor:_p}}>
                        PARSE WITH AI →
                      </button>
                    )}
                  </>
                ...
                {voiceStage==="recording" && (
                  <div style={{textAlign:"center"}}>
                    <div style={{...ss(10,600,"#C4A882",{letterSpacing:1.5,marginBottom:20})}}>● LISTENING…</div>

                    {/* Waveform */}
                    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4,height:40,marginBottom:20}}>
                      {[0,0.1,0.2,0.05,0.15,0.25,0.1,0.2,0.05,0.15,0.1,0.2,0.05].map((delay,i)=>(
                    ...
                    {/* Live transcript */}
                    {transcript && (
                      <div style={{background:_1a,borderRadius:14,padding:"14px 16px",marginBottom:20,textAlign:"left",minHeight:56}}>
                        <div style={ss(9,400,DM,{letterSpacing:1,marginBottom:6})}>TRANSCRIPT</div>
                        <div style={ss(12,400,MD,{lineHeight:1.7})}>{transcript}<span style={{opacity:0.5,animation:"pulse 1s infinite"}}>|</span></div>
                      </div>
                    )}

                    <button onClick={()=>{setVoiceStage("parsing");setTimeout(()=>{setVoiceStage("done");setScanned({name:"Double-Breasted Wool Blazer",brand:"Zara",category:"Outerwear",color:"#1E2A3A",price:120,tags:["smart","office","evening"],emoji:"🥼",wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",condition:"Like New",forSale:false});},2000);}}
                      style={{padding:"10px 28px",borderRadius:20,background:"#2A1A1A",border:"1px solid #4A2A2A",...ss(10,600,"#C4A0A0",{letterSpacing:1}),cursor:_p}}>
                      ■ STOP
                    </button>
                  </div>
                )}
                ...
                {voiceStage==="parsing" && (
                  <div style={{textAlign:"center",padding:"24px 0"}}>
                    ...
                    <div style={sr(16,400,G,{marginBottom:6})}>Parsing your description…</div>
                    <div style={ss(10,400,DM,{marginBottom:20})}>Identifying brand, category, color & condition</div>
                    <div style={{height:3,background:_1a,borderRadius:2,overflow:"hidden"}}>
                      <div style={{height:"100%",width:"60%",background:`linear-gradient(90deg,#8060C0,#6040A0)`,borderRadius:2,animation:"pulse 1s infinite"}}/>
                    </div>
                  </div>
                )}
              </div>
            ...
            {scanned && (
              <div style={{marginTop:addMode==="describe"?20:0}}>
                ...
                <div style={{display:"flex",gap:14,marginBottom:16}}>
                  <div style={{width:72,height:72,borderRadius:14,background:`linear-gradient(135deg,${scanned.color}22,${scanned.color}55)`,overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {photoPreview
                ...
                <div style={{display:"flex",gap:10}}>
                  <Btn onClick={()=>{setScanned(null);setVoiceStage("idle");setTranscript("");setVoiceDesc("");}} outline>RE-SCAN</Btn>
...
function OutfitsTab({items,outfits,setOutfits,setItems,showToast,logWear}){
  ...
  const save=()=>{
    ...
    setOutfits(prev=>[...prev,{id:newId,name:n,items:builder,occasion,season:"All Year"}]);
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",overflow:"hidden",transition:"background 0.3s,color 0.3s"}}>
      ...
      <div className="sc" style={{height:"calc(100vh - 130px)",paddingBottom:80}}>
        ...
        {tab==="closet"    && <ClosetTab items={items} setItems={setItems} setSelectedItem={setSelectedItem} showToast={showToast} wishlist={wishlist} setWishlist={setWishlist} />}
        {tab==="outfits"   && <OutfitsTab items={items} outfits={outfits} setOutfits={setOutfits} setItems={setItems} showToast={showToast} logWear={logWear} />}
        {tab==="market"    && (
