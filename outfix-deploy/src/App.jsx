                  ...
                  <button onClick={async()=>{
                    ...
                    try{
                      const raw = await callClaude(
                        `A user is describing a clothing item they want to add to their wardrobe app: "${voiceDesc}"\n\nGenerate 4 specific product matches they might be describing. For each, include the exact product page URL on the retailer's website (use real URLs from zara.com, cos.com, aritzia.com, net-a-porter.com, ssense.com, nordstrom.com or similar — use the actual product slug format for that retailer). Return ONLY JSON:\n{"results":[{"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":150,"emoji":"👚","condition":"New","tags":["..."],"productUrl":"https://...actual-product-page-url..."},...]}`
                      );
                      const json=JSON.parse(raw.replace(/```json|```/g,"").trim());
                      const results = json.results||[];
                      // Fetch real product images for each result in parallel
                      const withImages = await Promise.all(results.map(async r=>{
                        if(!r.productUrl) return {...r, imageUrl:null};
                        try{
                          const imgRes = await fetch("/api/fetch-image",{
                            method:"POST",
                            headers:{"Content-Type":"application/json"},
                            body:JSON.stringify({url:r.productUrl})
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    sb.select("items", session.access_token).then(data=>{
      ...
      // If no saved items, keep initItems (demo data)
      setClosetLoading(false);
    }).catch(()=>setClosetLoading(false));
  },[session]);
  ...
  const saveItemToDB = async (item) => {
    if(!session?.access_token) return;
    try {
      await sb.insert("items", session.access_token, {
