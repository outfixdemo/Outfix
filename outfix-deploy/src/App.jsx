      ...
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:20,border:`1px solid ${G}22`}}>
        ...
        <div
          ...
          <button onClick={e=>{
            ...
            // Update local event count so UI reflects immediately
            setLiveEvents(prev=>prev.map(e=>e.id===event.id?{...e,like_count:(e.like_count||0)+(isLiked?-1:1)}:e));
          }}
            style={{background:"none",border:"none",cursor:_p,display:"flex",alignItems:"center",gap:5,...ss(12,400,liked[event.id]?"#E08080":DM),flexShrink:0,padding:"4px 0"}}>
            {liked[event.id]?"♥":"♡"}
            <span style={ss(10,400,liked[event.id]?"#E08080":DM)}>{(event.like_count||0)+(liked[event.id]?1:0)}</span>
          </button>
        </div>
      </div>
    );
