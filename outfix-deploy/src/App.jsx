...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
  ...
  const closeAdd=()=>{setShowAdd(false);setScanned(null);setUrl("");setAddMode(null);setVoiceStage("idle");setTranscript("");setVoiceDesc("");setPhotoPreview(null);setDetectedItems([]);setSelectedDetected({});setDescribeResults([]);setDescribeLoading(false);setMName("");setMBrand("");setMPrice("");setMCat("Tops");setMDate("");setMCondition("Good");setMColor("#C4A882");};
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      <div style={{marginBottom:12}}>
        {/* Search bar */}
        <div style={{..._row,gap:10,background:CD,border:`1px solid ${closetSearch?G+"66":BR}`,borderRadius:12,padding:"8px 14px",marginBottom:10,cursor:"text"}}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {filtered.map(item=>{
          ...
          <div key={item.id} className="ch" onClick={()=>setSelectedItem(item)} style={{background:CD,borderRadius:16,overflow:"hidden",border:`1px solid ${BR}`,position:"relative"}}>
            <div style={{height:160,background:`linear-gradient(135deg,${item.color}22,${item.color}55)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:44,position:"relative",overflow:"hidden"}}>
              {item.sourceImage
                  ...
                  return(
                    <>
                      ...
                      <div style={{marginBottom:14}}>
                        ...
                        <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{if(e.target.files[0]){const r=new FileReader();r.onload=ev=>setPhotoPreview(ev.target.result);r.readAsDataURL(e.target.files[0]);}}}/>
                        {photoPreview ? (
                          ...
                          <button onClick={()=>fileRef.current.click()} style={{width:"100%",padding:"16px",borderRadius:14,background:_1a,border:`1px dashed ${G}44`,display:"flex",alignItems:"center",gap:12,cursor:_p}}>
                      ...
                      <div style={{marginBottom:12}}>
                        ...
                        <input value={mBrand} onChange={e=>setMBrand(e.target.value)} placeholder="e.g. Aritzia, Zara…" style={iStyle}/>
                      </div>
                      <div style={{marginBottom:12}}>
  ...
  return(
    <div style={{marginBottom:18}}>
      ...
      <div style={{display:"flex",gap:8,marginTop:6}}>
        ...
        <button onClick={()=>setShowSaveModal(true)} title="Save as outfit" style={{width:46,height:46,borderRadius:14,background:CD,border:`1px solid ${G}44`,display:"flex",alignItems:"center",justifyContent:"center",cursor:_p,flexShrink:0,...ss(18,300,G)}}>🪡</button>
        <button onClick={onNewLook} title="Build new look" style={{width:46,height:46,borderRadius:14,background:CD,border:`1px solid ${BR}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:_p,flexShrink:0,...ss(20,300,MD)}}>+</button>
      </div>
...
function WishlistAddModal({onClose, onAddToWishlist}){
  ...
  const fetchDesc=async()=>{
    ...
    try{
      const raw=await callClaude(`A user is describing an item they want for their wishlist: "${desc}"\nGenerate 4 specific matches. Return ONLY JSON: {"results":[{"name":"...","brand":"...","price":150,"emoji":"👗","color":"#hexcode","note":"..."}]}`);
      const json=JSON.parse(raw.replace(/```json|```/g,"").trim());
            ...
            <div style={{marginTop:8}}>
              ...
              {descResults.length>0&&(
                <div>
                  ...
                  {descResults.map((r,i)=>(
                    <div key={i} onClick={()=>{onAddToWishlist({id:Date.now()+i,name:r.name,brand:r.brand,price:r.price||0,emoji:r.emoji||"👗",gap:r.note||"Saved from description",inMarket:false,sourceImage:null,color:r.color||"#C4A882"});onClose();}}
                      ...
                      <div style={{width:52,height:52,borderRadius:12,background:`${r.color||G}22`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",border:_2a,fontSize:24}}>{r.emoji||"👗"}</div>
                      <div style={{flex:1}}>
