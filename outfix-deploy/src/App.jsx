...
function Toast({msg}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",
      ...
      ...ss(11,600,BK,{letterSpacing:1,textAlign:"center"}),
      padding:"14px 24px",zIndex:9998,
...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
  ...
  const [scanCropSrc,setScanCropSrc]=useState(null); // now only used for manual photo crop before save
  const [cropBeforeSaveItem,setCropBeforeSaveItem]=useState(null); // holds scanned item waiting for final crop step
          ...
          {scanCropSrc&&(
            <div onClick={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()}>
              <CropModal src={scanCropSrc} onCancel={()=>setScanCropSrc(null)}
                onSave={cropped=>{setScanCropSrc(null);setPhotoPreview(cropped);confirmAdd(cropped);}} autoRemoveBg={false}/>
            </div>
          )}
              ...
              <div style={{marginTop:16}}>
                {(()=>{
                  ...
                  const confirmManual=()=>{
                    ...
                    if(photoPreview) setScanCropSrc(photoPreview);
                  };
                  return(
                    <>
            ...
            {scanned && (
              <div style={{marginTop:addMode==="describe"?20:0}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <Btn onClick={()=>{
                    const src = scannedItem?._originalPhoto || photoPreview;
                    if(src) setScanCropSrc(src);
                    else confirmAdd();
                  }} full>CROP & ADD</Btn>
                </div>
              </div>
  ...
  return(
    ...
    >
      ...
      <div style={{width:"100%",height:imgZoneH,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",flexShrink:0}}>
        ...
        >
          <img ref={imgRef} src={src} style={{width:displaySize.w,height:displaySize.h,objectFit:"contain",display:"block",userSelect:"none",pointerEvents:"none",borderRadius:8}} alt="crop"/>
      ...
      <div style={{flexShrink:0,marginTop:12}}>
        ...
        />
        {bgError&&(
          <div style={{marginBottom:8,padding:"7px 12px",borderRadius:10,background:"#2A1A1A",border:"1px solid #CC333344",...ss(9,500,"#CC6666",{textAlign:"center"})}}>
            {bgError}
          </div>
        )}
        <div style={{display:"flex",gap:8}}>
...
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit,styleProfile={},saveStyleProfile,postWearFeedEvent}){
  ...
  return(
    <div style={{marginBottom:18}}>
      ...
      {showSaveModal&&(
        <div onClick={()=>setShowSaveModal(false)} style={{..._fix,background:"#000000CC",zIndex:80,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:24,width:"100%",maxWidth:382,border:_2a,padding:"24px"}}>
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {selectedOutfit&&(()=>{
        ...
        return(
          <div style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#0D0D0D",zIndex:70,display:"flex",flexDirection:"column"}}>
            {/* Pinned header */}
            <div style={{flexShrink:0,padding:"16px 20px 12px",borderBottom:"1px solid #1E1E1E"}}>
              <div style={{..._btwnS,marginBottom:10}}>
                <div>
                ...
                <IconBtn onClick={()=>setSelectedOutfit(null)}>x</IconBtn>
              </div>
              <div style={{display:"flex",gap:10}}>
            ...
            {/* Scrollable body - starts at top */}
            <div className="sc" style={{flex:1,overflowY:"auto",padding:"16px 20px 40px"}}>
...
Respond ONLY with JSON: {"days":[{"day":1,"date":"${tripData.startDate}","label":"Arrival & Check-in","activity":"Travel","emoji":"✈️"},...]}`
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      <div onTouchStart={handleVacSwipeStart} onTouchEnd={handleVacSwipeEnd}
        ...
        {weather&&!wxLoading&&(
          <div style={{marginTop:14}}>
            ...
            {/* Seasonal summary text — shown for far-out trips, no day icons */}
            ...
            {/* Day cards — only shown for real forecasts (within 14 days), with actual dates */}
            {!weather.seasonal&&(
      ...
      {confirmDeleteTrip&&(
        <div onClick={()=>setConfirmDeleteTrip(false)} style={{..._fix,inset:0,background:"#000000CC",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:24,width:"100%",maxWidth:382,padding:"28px 24px",border:"1px solid #2A2A2A"}}>
...
function CalendarTab({outfits,items,showToast,logWear,events,setEvents,session}){
  ...
  const saveEventToDB=async(ev)=>{
    ...
    const headers={"Content-Type":"application/json","Authorization":`Bearer ${session.access_token}`,"apikey":SB_KEY,"Prefer":"resolution=merge-duplicates"};
    await fetch(`${SB_URL}/rest/v1/calendar_events`,{method:"POST",headers,body:JSON.stringify({
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {/* ── ADD EVENT MODAL ── */}
      {showAddEvent&&(
        <div style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#0D0D0D",zIndex:80,display:"flex",flexDirection:"column"}}>
            ...
            <div style={{flexShrink:0,padding:"16px 24px 12px"}}>
              ...
              <div style={{..._btwn}}>
                <div>
                  <div style={sr(20,400)}>Add Event</div>
                  <div style={ss(9,400,DM,{letterSpacing:1,marginTop:2})}>PLAN YOUR UPCOMING OCCASION</div>
                </div>
                <button onClick={()=>setShowAddEvent(false)} style={{width:30,height:30,borderRadius:"50%",background:_1a,border:_2a,cursor:_p,...ss(14,400,MD),display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
              </div>
            </div>
            {/* Scrollable body */}
            <div className="sc" style={{flex:1,overflowY:"auto",padding:"0 24px 40px"}}>
              ...
              <div style={{marginBottom:20}}>
                <SingleDatePicker label="DATE" value={newDate} onChange={setNewDate}/>
              </div>
              <div style={{display:"flex",gap:10}}>
  ...
  return(
    <div style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#0D0D0D",zIndex:110,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flexShrink:0,padding:"16px 24px 12px",borderBottom:`1px solid ${BR}`,display:"flex",gap:14,alignItems:"center"}}>
        ...
        <button onClick={onClose} style={{background:"none",border:"none",cursor:_p,fontSize:22,color:DM,lineHeight:1}}>x</button>
      </div>
      {/* Scrollable body */}
