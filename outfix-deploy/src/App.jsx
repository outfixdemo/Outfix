...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  const loadLiveFeed = async () => {
    ...
    try {
      ...
      if(followingIds.length === 0){ setFeedLoading(false); return; }
      // Load their recent feed events + their profile usernames
      ...
      if(Array.isArray(eventsData)){
        const mapped = eventsData.map(e=>({
          ...e,
          username: profileMap[e.user_id]?.username || null,
  ...
  const loadSuggestedAccounts = async () => {
    ...
    try {
      ...
      if(myFollowerIds.length === 0){ setSuggestedLoading(false); return; }

      // Get who my followers follow (potential suggestions)
      const followerIdStr = myFollowerIds.join(",");
      const mutualCandidatesRes = await fetch(`${SB_URL}/rest/v1/follows?follower_id=in.(${followerIdStr})&select=follower_id,following_id`, {headers}).then(r=>r.json());

      // Count mutual connections, exclude myself and people I already follow
      const mutualCount = {};
      if(Array.isArray(mutualCandidatesRes)){
        mutualCandidatesRes.forEach(f=>{
          const candidate = f.following_id;
          if(candidate === myId) return;
          if(myFollowingIds.includes(candidate)) return;
  ...
  const LiveEventCard = ({event}) => {
    ...
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:16,border:`1px solid ${G}22`}}>
        ...
        {/* User row */}
        <div
          onClick={e=>{e.stopPropagation();e.preventDefault();setViewProfile({userId:event.profileUserId,username:event.username});}}
          style={{display:"flex",gap:12,alignItems:"center",padding:"12px 16px",cursor:_p}}>
          <div style={{width:36,height:36,borderRadius:"50%",background:"#2A2A2A",overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>
            👤
          </div>
          <div style={{flex:1}}>
            <div style={ss(11,600,G,{letterSpacing:0.5})}>
              {event.username ? `@${event.username}` : "Outfix user"}
            </div>
  ...
  return(
    <div ref={feedRef} className="fu" style={{padding:"0 16px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {/* Instagram-style pull-to-refresh spinner — fixed overlay, never adds scroll height */}
      {(pullProgress > 0 || refreshing) && (
        <div style={{position:"fixed",top:58,left:0,right:0,maxWidth:430,margin:"0 auto",zIndex:50,display:"flex",justifyContent:"center",pointerEvents:"none"}}>
          <div style={{
            width:32,height:32,borderRadius:"50%",
            background:"#1A1A1A",border:`1px solid ${G}33`,
            display:"flex",alignItems:"center",justifyContent:"center",
            boxShadow:"0 2px 8px #00000066",
            transform:`translateY(${refreshing?0:pullProgress*20-8}px)`,
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button className="sb" onClick={async()=>{
                      ...
                      try{
                        ...
                        // Build a rich prompt using scraped data if available
                        const scrapedContext = [
                          productRes.name ? `Product title: "${productRes.name}"` : null,
                          productRes.brand ? `Brand: "${productRes.brand}"` : null,
                          productRes.description ? `Description: "${productRes.description?.slice(0,200)}"` : null,
                        ].filter(Boolean).join("\n");
                        const prompt = scrapedContext
                          ? `A user is adding a clothing item to their wardrobe app.\nURL: "${url}"\n${scrapedContext}\n\nUsing this information, identify the item precisely. Return ONLY JSON: {"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":${productRes.price||150},"tags":["..."],"emoji":"👚","condition":"Like New"}`
                          : `A user pasted this product URL into a wardrobe app: "${url}"\n\nInfer brand from domain, item name and category from the URL path. Return ONLY JSON: {"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":150,"tags":["..."],"emoji":"👚","condition":"Like New"}`;
                        ...
                        // If no image from scraper, try Google Image Search with exact scraped/AI name
                        let finalImage = productRes.image || null;
                        if(!finalImage){
                          try{
                            const imgQuery = `${json.brand} ${productRes.name||json.name} official product photo`;
                            const imgRes = await fetch("/api/image-search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:imgQuery})}).then(x=>x.json()).catch(()=>({imageUrl:null}));
                            finalImage = imgRes.imageUrl || null;
                          }catch(e){}
                        }
                        setScanning(false);
...
export default function App(){
  ...
  const logWear = (outfitId) => {
    ...
    if(session?.access_token && wornOutfit) {
      const uid = session.user?.id;
      const outfitItemObjs = (wornOutfit.items||[]).map(id=>items.find(i=>i.id===id)).filter(Boolean);
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",overflowX:"hidden",transition:"background 0.3s,color 0.3s"}}>
      <style>{GCSS}</style>
      ...
      {/* ── CONTENT ── */}
      <div id="main-scroll" className="sc" style={{height:"calc(100vh - 130px)",paddingBottom:80}}>
