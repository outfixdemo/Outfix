...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  const loadLiveFeed = async () => {
    ...
    try {
      ...
      // Get who I follow, then include myself in the feed
      ...
      // Always include own events even if following nobody
      const allIds = [...new Set([uid, ...followingIds])];
      const ids = allIds.join(",");
      const [eventsData, profilesData] = await Promise.all([
        fetch(`${SB_URL}/rest/v1/feed_events?user_id=in.(${ids})&order=created_at.desc&limit=30`, {
          headers: {...sbHeaders(session.access_token)}
        }).then(r=>r.json()),
  ...
  loadFeedRef.current = loadLiveFeed;
  ...
  const LiveEventCard = ({event}) => {
    ...
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:20,border:`1px solid ${G}22`}}>
        {/* Hero — clickable for added_item events */}
        <div onClick={eventItem ? ()=>setSelectedFeedItem(eventItem) : undefined}
          style={{width:"100%",position:"relative",cursor:eventItem?_p:"default"}}>
          <div style={{width:"100%",paddingTop:isWore?"90%":"100%",position:"relative",overflow:"hidden"}}>
            {!isWore && event.item_image ? (
              ...
              <div style={{..._abs0,background:`linear-gradient(135deg,${G}12,${G}28)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8}}>
                {isWore ? (
                  <div style={{display:"flex",gap:16,fontSize:56}}>{emojis.length ? emojis.map((e,i)=><span key={i}>{e}</span>) : ["👗","👟","✨"].map((e,i)=><span key={i}>{e}</span>)}</div>
                ) : (
                  <div style={{fontSize:72}}>{event.item_emoji||"👗"}</div>
                )}
              </div>
            )}
          </div>
        ...
        <div
          ...
          <div style={{width:36,height:36,borderRadius:"50%",background:event.isOwn?`${G}33`:"#2A2A2A",border:event.isOwn?`1px solid ${G}55`:"none",overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>
            {event.isOwn ? "✦" : "👤"}
          </div>
          <div style={{flex:1}}>
            <div style={ss(11,600,event.isOwn?G:MD,{letterSpacing:0.5})}>
              {event.isOwn ? "You" : (event.username ? `@${event.username}` : "Outfix user")}
            </div>
            <div style={ss(9,400,DM)}>{event.timeAgo}</div>
          </div>
          <div style={ss(9,400,DM,{fontStyle:"italic"})}>
  ...
  const PostCard = ({feed})=>{
    const accent = POST_ACCENTS[(feed.id-1)%POST_ACCENTS.length];
    const emojis = feed.items?.slice(0,4).map(i=>i.emoji).filter(Boolean) || ["👗","👟","✨"];
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:20,border:`1px solid ${BR}`}}>
        {/* Hero card */}
        <div style={{width:"100%",position:"relative",background:_1a}}>
          <div style={{width:"100%",paddingTop:"110%",position:"relative",overflow:"hidden"}}>
            <div style={{..._abs0,background:`linear-gradient(135deg,${accent}18,${accent}38)`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{display:"flex",gap:20,fontSize:62}}>{emojis.map((e,i)=><span key={i}>{e}</span>)}</div>
            </div>
          </div>
        ...
        <div style={{padding:"12px 16px 0"}}>
          <div style={{display:"flex",gap:10,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
            {feed.items.map((item,i)=>{
              return(
                <div key={i} onClick={e=>{e.stopPropagation();setSelectedFeedItem(item);}}
                  style={{flexShrink:0,width:96,borderRadius:16,overflow:"hidden",border:"1.5px solid #2A2A2A",background:"#1A1A1A",cursor:_p}}>
                  <div style={{height:80,display:"flex",alignItems:"center",justifyContent:"center",background:"#1E1E1E",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{padding:"8px 8px 10px"}}>
                    <div style={ss(9,400,MD,{lineHeight:1.3,marginBottom:2,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"})}>{item.name}</div>
    ...
    <div ref={feedRef} className="fu" style={{padding:"0 8px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {liveEvents.length > 0 && (
        <div style={{marginBottom:4}}>
          ...
          {liveEvents.slice(0,10).map(e=><LiveEventCard key={e.id} event={e}/>)}
          {liveEvents.length > 10 && <div style={ss(9,400,DM,{textAlign:"center",marginBottom:16,fontStyle:"italic"})}>Pull down to see more</div>}
      ...
      {selectedFeedItem&&(
        <div onClick={()=>setSelectedFeedItem(null)} style={{position:"fixed",top:76,left:0,right:0,bottom:0,background:"#000C",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 16px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:24,width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${G}22`}}>
            <div style={{display:"flex",justifyContent:"flex-end",padding:"12px 16px 0"}}>
              <button onClick={()=>setSelectedFeedItem(null)} style={{width:28,height:28,borderRadius:"50%",background:"#1A1A1A",border:"1px solid #2A2A2A",cursor:_p,...ss(13,400,DM),display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
            <div style={{width:"100%",height:240,background:`linear-gradient(135deg,${G}12,${G}28)`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
              {selectedFeedItem.sourceImage
                ...
                : <div style={{fontSize:selectedFeedItem._isOutfit?52:72}}>{selectedFeedItem.emoji}</div>
              }
            </div>
            <div style={{padding:"16px 20px 36px"}}>
              <div style={sr(22,500,undefined,{marginBottom:4})}>{selectedFeedItem.name}</div>
              {selectedFeedItem._isOutfit ? (
                <>
                  <div style={ss(9,400,DM,{marginBottom:14})}>Items in this outfit</div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:20}}>
                    {(selectedFeedItem.tags||[]).map((t,i)=>(
                      <div key={i} style={{background:"#1A1A1A",borderRadius:8,padding:"4px 12px",border:"1px solid #2A2A2A",...ss(10,400,MD)}}>{t}</div>
                    ))}
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div style={{display:"flex",gap:10,marginBottom:20}}>
                    {[
                      [selectedFeedItem.price?`$${selectedFeedItem.price}`:"—","PRICE"],
                      [selectedFeedItem.category||"—","CATEGORY"],
                      [selectedFeedItem.condition||"—","CONDITION"]
                    ].map(([v,l])=>(
                  ...
                  <div style={{display:"flex",gap:10}}>
                    <button onClick={()=>{handleWishlist(selectedFeedItem);setSelectedFeedItem(null);}} style={{flex:1,padding:"14px",borderRadius:16,background:"#1A1A2A",border:`1px solid ${G}44`,...ss(10,600,G,{letterSpacing:1}),cursor:_p}}>♡ WISHLIST</button>
                    <button onClick={()=>{handleAddToCloset(selectedFeedItem);setSelectedFeedItem(null);}} style={{flex:1,padding:"14px",borderRadius:16,background:`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(10,700,BK,{letterSpacing:1.5}),cursor:_p}}>+ CLOSET</button>
                  </div>
                </>
              )}
            </div>
...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
  ...
  const closeAdd=()=>{setShowAdd(false);setScanned(null);setUrl("");setAddMode(null);setVoiceDesc("");setPhotoPreview(null);setManualCropSrc(null);setDetectedItems([]);setSelectedDetected({});setDescribeResults([]);setDescribeLoading(false);setMName("");setMBrand("");setMPrice("");setMCat("Tops");setMDate("");setMCondition("Good");setMColor("#C4A882");setShowBrandList(false);};
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button className="sb" onClick={async()=>{
                      ...
                      try{
                        ...
                        // Extract URL slug as fallback context
                        let slugContext = "";
                        try {
...
Instructions:
- Infer the brand from the domain (e.g. revolve.com → check slug for brand name, zara.com → Zara)
- Parse the URL slug carefully: hyphens separate words, brand often appears first (e.g. "line-dot-belmont-blazer-in-brown" → brand "Line + Dot", name "Belmont Blazer in Brown")
- For revolve.com URLs, the slug format is typically "brand-name-product-name"
- Pick the most accurate color hex based on color words in the slug or title
...
{"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":${scraped.price||0},"tags":["..."],"emoji":"👚","condition":"Like New"}`;
                        ...
                        // Image: use scraped first, then retailer-aware extraction, then search fallback
                        let finalImage = scraped.image || null;
                        if(!finalImage){
                          // Ask Claude to suggest a likely image URL based on the product URL pattern
                          try{
                            const imgPrompt = `Given this product URL: "${url.trim()}"
And product name: "${json.name}", brand: "${json.brand}"
If this is a known retailer with predictable image CDN patterns (e.g. revolve, zara, asos, nordstrom, net-a-porter), suggest the most likely direct product image URL.
Otherwise return null.
Return ONLY a JSON object: {"imageUrl": "https://..." } or {"imageUrl": null}`;
                            const imgRaw = await callClaude(imgPrompt);
                            const imgJson = JSON.parse(imgRaw.replace(/```json|```/g,"").trim());
                            if(imgJson.imageUrl) finalImage = imgJson.imageUrl;
                          }catch(e){}
                        }
                        if(!finalImage){
                          // Progressive image search fallback
                          const hostname = (() => { try { return new URL(url.trim()).hostname.replace("www.",""); } catch(e){ return ""; } })();
                          const queries = [
                            `${json.brand} ${json.name} ${hostname}`,
                            `${json.brand} ${json.name} product photo`,
                            `${json.name} ${json.brand}`,
                          ];
...
// ── MIX & MATCH BUILDER ──────────────────────────────────────────────────────
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit,styleProfile={},saveStyleProfile}){
...
function OutfitsTab({items,outfits,setOutfits,setItems,showToast,logWear,onSaveOutfit,onDeleteOutfit,styleProfile={},saveStyleProfile}){
    ...
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {/* EDIT TRIP SHEET */}
        <div onClick={()=>setEditingTrip(false)} style={{..._fix,inset:0,background:"#000000BB",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,margin:"0 auto",padding:"20px 24px 40px",border:"1px solid #2A2A2A"}}>
            <div style={{width:36,height:4,borderRadius:2,background:"#333",margin:"0 auto 20px"}}/>
...
function SettingsTab({currentPlan,setShowPricing,showToast,items,userName="",userEmail="",onSignOut,userProfile={},saveProfile,styleProfile={},saveStyleProfile}){
...
export default function App(){
  ...
  const logWear = (outfitId) => {
    ...
    if(session?.access_token && wornOutfit) {
      ...
      if(uid) sb.insert("feed_events", session.access_token, {
        ...
        item_emojis: outfitItemObjs.map(i=>i.emoji).filter(Boolean).slice(0,4),
        item_names: outfitItemObjs.map(i=>i.name).slice(0,4),
      }).catch(()=>{});
    }
  };
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",transition:"background 0.3s,color 0.3s"}}>
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,maxWidth:430,margin:"0 auto",padding:"20px 24px 14px",background:hdrBg,zIndex:20,transition:"background 0.3s",borderBottom:`1px solid #1A1A1A`}}>
        <div style={{..._btwn}}>
          ...
          <div style={{..._row,gap:8}}>
            ...
            <button className="tb" onClick={()=>setShowPushNotifs(true)} style={{width:30,height:30,borderRadius:"50%",border:`1px solid ${divLine}`,display:"flex",alignItems:"center",justifyContent:"center",background:"none",cursor:_p,position:"relative"}}>
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                ...
                <circle cx="14" cy="5" r="3" fill="#C46060" stroke="#0D0D0D" strokeWidth="1"/>
              </svg>
            ...
            <button className="tb" onClick={()=>setTab("__settings")} style={{width:30,height:30,borderRadius:"50%",border:`1px solid ${divLine}`,display:"flex",alignItems:"center",justifyContent:"center",background:"none",cursor:_p}}>
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      ...
      <div id="main-scroll" className="sc" style={{height:"100vh",paddingTop:76,paddingBottom:80,boxSizing:"border-box"}}>
        ...
        {tab==="outfits"   && <OutfitsTab items={items} outfits={outfits} setOutfits={setOutfits} setItems={setItems} showToast={showToast} logWear={logWear} onSaveOutfit={saveOutfitToDB} onDeleteOutfit={deleteOutfitFromDB} styleProfile={styleProfile} saveStyleProfile={saveStyleProfile}/>}
        {tab==="market"    && (
        ...
        <div onClick={()=>setTab("home")} style={{..._fix,background:"#000000BB",zIndex:90,display:"flex",alignItems:"flex-start",paddingTop:60}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"0 0 24px 24px",width:"100%",maxWidth:430,margin:"0 auto",border:_2a,animation:"fadeDown 0.3s ease",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
            ...
            <div className="sc" style={{overflowY:"auto",flex:1}}>
              <SettingsTab currentPlan={currentPlan} setShowPricing={setShowPricing} showToast={showToast} items={items} userName={userName} userEmail={userEmail} onSignOut={handleSignOut} userProfile={userProfile} saveProfile={saveProfile} styleProfile={styleProfile} saveStyleProfile={saveStyleProfile}/>
            </div>
          </div>
        </div>
      )}
