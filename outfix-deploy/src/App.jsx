...
// Sharpens the alpha channel to eliminate fog caused by the model's uncertainty on
// same-color backgrounds (white shirt on white bg, etc.).
// Mid-alpha "mush" (the fog) gets snapped to 0 or 255 via a smooth curve.
  ...
  return new Promise(resolve=>{
    ...
    img.onload=()=>{
      ...
      for(let i=0;i<w*h;i++){
        ...
        // Only erase pixels that are BOTH low-alpha AND near-white in color.
        // This catches background fog without touching shirt fabric:
        // - White shirt fabric: high alpha (200+) → untouched
        // - Shirt soft edges: mid alpha but colored → untouched
        // - White bg fog: low alpha (<70) AND near-white (min channel >200) → erase
        const whiteness=Math.min(r,g,b);
        if(a<70&&whiteness>200) d[i*4+3]=0;
      }
