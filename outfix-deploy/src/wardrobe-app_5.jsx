...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Load items, outfits, and wishlist in parallel
    Promise.all([
      sb.select("items", session.access_token),
      sb.select("outfits", session.access_token),
      sb.select("wishlist", session.access_token),
    ]).then(([itemData, outfitData, wishlistData])=>{
      ...
      if(itemData?.code || itemData?.error){
        ...
        // Seed demo items into DB so wear counts persist going forward
        ...
        if(userId) {
          Promise.all(initItems.map(item =>
            ...
            // Use DB-assigned IDs so updateWearInDB can find the rows
