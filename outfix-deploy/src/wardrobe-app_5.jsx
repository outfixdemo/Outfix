...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
    ...
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:20,border:`1px solid ${G}22`}}>
        ...
        <div onClick={eventItem ? ()=>setSelectedFeedItem(eventItem) : undefined}
          ...
          <div style={{width:"100%",paddingTop:isWore?"115%":"125%",position:"relative",overflow:"hidden"}}>
            {!isWore && event.item_image ? (
              ...
              <div style={{..._abs0,background:`linear-gradient(135deg,${G}08,${G}18)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,padding:"20px 24px",boxSizing:"border-box"}}>
                {isWore ? (
                  // Stacked item cards like outfit page
                  <div style={{width:"100%",display:"flex",flexDirection:"column",gap:10}}>
                    {(event.item_names||[]).slice(0,3).map((name,i)=>{
                      const img = (event.item_images||[])[i];
                      const color = (event.item_colors||[])[i] || "#2A2A2A";
                      const emoji = (event.item_emojis||[])[i] || "👗";
                      return(
                        <div key={i} style={{background:`linear-gradient(135deg,${color}22,${color}44)`,borderRadius:16,border:`1px solid ${color}44`,display:"flex",alignItems:"center",gap:14,padding:"10px 14px",height:72}}>
                          <div style={{width:52,height:52,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:10,background:`${color}22`,overflow:"hidden"}}>
                            {img
                              ? <img src={img} style={{width:"100%",height:"100%",objectFit:"contain"}} alt={name}/>
                              : <span style={{fontSize:28}}>{emoji}</span>
                            }
                          </div>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={sr(15,500,"#F0EBE3",{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"})}>{name}</div>
                            <div style={ss(9,400,DM,{marginTop:2})}>{(event.item_brands||[])[i]||""}</div>
                          </div>
                        </div>
                      );
                    })}
        ...
        <div
          ...
          {/* Like button */}
          <button onClick={e=>{e.stopPropagation();setLiked(p=>({...p,[event.id]:!p[event.id]}));}}
            style={{background:"none",border:"none",cursor:_p,display:"flex",alignItems:"center",gap:4,...ss(12,400,liked[event.id]?"#E08080":DM),flexShrink:0,padding:"4px 0"}}>
            {liked[event.id]?"♥":"♡"}
          </button>
        </div>
      </div>
  ...
  return(
    <div ref={feedRef} className="fu" style={{padding:"0 8px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {/* ── Feed: trend cards only in Discover ── */}
      <EventCard/>
      ...
      {selectedFeedItem&&(
        <div onClick={()=>setSelectedFeedItem(null)} style={{position:"fixed",inset:0,background:"#000C",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",width:"100%",maxWidth:430,height:"100%",overflowY:"auto",display:"flex",flexDirection:"column"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"56px 20px 12px"}}>
              <div style={sr(20,400)}>{selectedFeedItem._isOutfit?"Outfit":"Item"}</div>
              <button onClick={()=>setSelectedFeedItem(null)} style={{width:32,height:32,borderRadius:"50%",background:"#1A1A1A",border:"1px solid #2A2A2A",cursor:_p,...ss(16,400,DM),display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
            <div style={{width:"100%",flex:"0 0 340px",background:`linear-gradient(135deg,${G}12,${G}28)`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
              {selectedFeedItem.sourceImage
                ? <img src={selectedFeedItem.sourceImage} style={{width:"100%",height:"100%",objectFit:"contain",padding:24,boxSizing:"border-box"}} alt={selectedFeedItem.name}/>
                : <div style={{fontSize:selectedFeedItem._isOutfit?64:88}}>{selectedFeedItem.emoji}</div>
              }
            </div>
