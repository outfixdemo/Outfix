            ...
            const persist = async () => {
              ...
              if(sourceImageUrl){
                const userId2 = session.user?.id ||
                  (() => { try { return JSON.parse(atob(session.access_token.split(".")[1])).sub; } catch(e){ return null; } })();
                if(userId2){
                  const oldImageUrl = updated._oldSourceImage; // passed from onUpdate
                  // Strategy 1: match by old image URL (most reliable)
                  // Strategy 2: match by user_id + item_name as fallback
                  const queries = [];
                  if(oldImageUrl && oldImageUrl.startsWith("http")){
                    queries.push(fetch(`${SB_URL}/rest/v1/feed_events?user_id=eq.${userId2}&item_image=eq.${encodeURIComponent(oldImageUrl)}`,{
                      method:"PATCH",
                      headers:{...sbHeaders(session.access_token),"Prefer":"return=representation"},
                      body:JSON.stringify({item_image:sourceImageUrl}),
                    }));
                  }
                  // Always also try by name as belt-and-suspenders
                  if(updated.name){
                    queries.push(fetch(`${SB_URL}/rest/v1/feed_events?user_id=eq.${userId2}&item_name=eq.${encodeURIComponent(updated.name)}`,{
                      method:"PATCH",
                      headers:{...sbHeaders(session.access_token),"Prefer":"return=representation"},
                      body:JSON.stringify({item_image:sourceImageUrl}),
                    }));
                  ...
                  console.log("feed sync total rows updated:", totalUpdated, results);
                  if(totalUpdated > 0){
                    showToast(`Feed updated \u2746`);
                    setLiveEvents(prev=>prev.map(e=>
                      (e.user_id===userId2 && (e.item_image===oldImageUrl || e.item_name===updated.name))
                        ? {...e, item_image:sourceImageUrl}
                        : e
                    ));
                  } else {
                    showToast(`No matching feed posts found`);
                    console.log("No feed events matched. userId:", userId2, "oldImage:", oldImageUrl, "name:", updated.name);
                  }
                }
