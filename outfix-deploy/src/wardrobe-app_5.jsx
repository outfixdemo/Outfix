...
function CropModal({src, onCancel, onSave}){
  ...
  const applyWithBgRemoval=async()=>{
    ...
    // buildCroppedB64 can fall back to raw URL on CORS failure — API needs base64
    ...
    try{
      ...
      if(resp.status===503){
        setBgError("Model warming up, retrying in 10s\u2026");
        await new Promise(r=>setTimeout(r,10000));
        setBgError(null);
        const retry=await fetch("/api/remove-bg",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({imageBase64:croppedB64})});
        const retryData=await retry.json().catch(()=>({}));
        if(retryData.image){ onSave(retryData.image); return; }
        setRemovingBg(false);
      ...
      if(!resp.ok){
        setRemovingBg(false);
        setBgError("Error "+resp.status+" — check that remove-bg.js is deployed.");
        return;
      }
      const data=await resp.json();
      if(data.image){ onSave(data.image); }
      else{ setRemovingBg(false); setBgError("No result from API. Try again."); }
    }catch(e){
      setRemovingBg(false);
      setBgError("Network error — check your connection.");
    }
  };
