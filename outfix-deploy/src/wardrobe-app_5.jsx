...
function CropModal({src, onCancel, onSave}){
  ...
  const applyWithBgRemoval=async()=>{
    ...
    try{
      ...
      // 503 = model cold-starting on HF — retry once after 10s
      if(resp.status===503 && data.loading){
        ...
        onSave(retryData.image||croppedB64);
      } else {
        onSave(data.image||croppedB64);
      }
    }catch(e){ onSave(croppedB64); }
    setRemovingBg(false);
  };
