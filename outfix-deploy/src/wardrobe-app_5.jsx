  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {filtered.length===0?(
        ...
        return(
          <div key={outfit.id} className="ch" onClick={()=>setSelectedOutfit(outfit)} style={{background:CD,borderRadius:16,padding:"12px",marginBottom:0,border:`1px solid ${BR}`,position:"relative",overflow:"hidden",cursor:_p}}>
            ...
            {/* Top row */}
            <div style={{..._btwnS,marginBottom:8,paddingLeft:6}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{..._row,gap:4,marginBottom:2}}>
                  {isPinned&&<span style={{fontSize:9}}>📌</span>}
                  <div style={ss(12,500,"#E8E0D4",{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"})}>{outfit.name}</div>
                </div>
                <div style={{background:accentCol+"33",borderRadius:6,padding:"1px 6px",display:"inline-block",...ss(7,600,accentCol==="#4A4038"?MD:accentCol,{letterSpacing:0.5})}}>{outfit.occasion}</div>
              </div>
              <button onClick={e=>{e.stopPropagation();toggleFav(outfit.id);}} style={{width:24,height:24,borderRadius:"50%",background:"transparent",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:_p,fontSize:12,flexShrink:0}}>
                <span style={{color:isFav?G:"#3A3028"}}>{isFav?"♥":"♡"}</span>
              </button>
            </div>
            ...
            <div style={{display:"flex",gap:5,marginBottom:10,paddingLeft:6,flexWrap:"wrap"}}>
              {outfit.items.slice(0,3).map(id=>{
                ...
                return it?(<ItemThumb key={id} item={it} size={38} r={9}/>):null;
              })}
              {outfit.items.length>3&&<div style={{width:38,height:38,borderRadius:9,background:_1a,border:_2a,display:"flex",alignItems:"center",justifyContent:"center",...ss(9,600,DM)}}>+{outfit.items.length-3}</div>}
            </div>

            {/* Wear Today button */}
            <button onClick={e=>{e.stopPropagation();logWear(outfit.id);showToast(`Wearing "${outfit.name}" today \u2746`);}} style={{width:"100%",padding:"6px",borderRadius:9,background:`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(8,600,BK,{letterSpacing:0.8}),cursor:_p}}>WEAR TODAY</button>
          </div>
        );
        })}
        </div>
