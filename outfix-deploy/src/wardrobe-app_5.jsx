            ...
            const persist = async () => {
              ...
              if(sourceImageUrl){
                ...
                if(userId2 && updated.name){
                  // Match by user_id + item_name — catches all events regardless of whether item_id was stored
                  fetch(`${SB_URL}/rest/v1/feed_events?user_id=eq.${userId2}&item_name=eq.${encodeURIComponent(updated.name)}`,{
                    method:"PATCH",
                    headers:{...sbHeaders(session.access_token),"Prefer":"return=representation"},
                    body:JSON.stringify({item_image:sourceImageUrl}),
                  }).then(async r=>{
                    const body = await r.json().catch(()=>({}));
                    console.log("feed sync status:",r.status,"rows updated:",Array.isArray(body)?body.length:body);
                    if(r.status===200 && Array.isArray(body) && body.length>0){
                      showToast(`Feed updated \u2746`);
                    } else if(r.status===200 && Array.isArray(body) && body.length===0){
                      showToast(`No feed posts found for "${updated.name}"`);
                    } else {
                      showToast(`Feed sync issue: ${r.status}`);
                    }
                  }).catch(e=>{ console.error("feed sync error:",e); showToast("Feed sync failed"); });
                }
