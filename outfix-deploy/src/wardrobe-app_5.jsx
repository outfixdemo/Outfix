...
function MonthYearPicker({value, onChange, label}){
  // value: "Jan 2024" or "" — parse it
  const parts = value ? value.split(" ") : [];
  const initMonth = parts[0] ? MONTHS_SHORT.indexOf(parts[0]) : -1;
  const initYear  = parts[1] ? parseInt(parts[1]) : -1;
  const [month,setMonth]=useState(initMonth>=0?initMonth:-1);
  const [year,setYear]=useState(initYear>0?initYear:-1);
...
function FullDatePicker({value, onChange, label}){
  // value: e.g. "Fri Mar 21" or "Mar 21" or "2026-03-21" — try to parse
  const tryParse=()=>{
    if(!value) return {d:-1,m:-1,y:-1};
    const iso=value.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if(iso) return {d:parseInt(iso[3]),m:parseInt(iso[2])-1,y:parseInt(iso[1])};
    const parts=value.replace(/^[A-Za-z]+\s/,"").split(" "); // strip weekday
    const mIdx=MONTHS_SHORT.findIndex(ms=>parts[0]?.startsWith(ms));
    const d=parseInt(parts[1]);
    return {d:isNaN(d)?-1:d, m:mIdx, y:-1};
  };
  const parsed=tryParse();
  const [day,setDay]=useState(parsed.d>0?parsed.d:-1);
  const [month,setMonth]=useState(parsed.m>=0?parsed.m:-1);
  const [year,setYear]=useState(parsed.y>0?parsed.y:-1);
...
// ── HOME ─────────────────────────────────────────────────────────────────────
// ── STORY VIEWER ─────────────────────────────────────────────────────────────
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems}){
...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
    ...
    if(file && file.type.startsWith("image/")){
      ...
      reader.onload=async(e)=>{
        ...
        try{
          ...
          setScanning(false);
          if(found.length===1){
            setScannedItem({...found[0],wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",forSale:false});
  ...
  const startRecording=()=>{
    ...
    setTimeout(()=>{
      setVoiceStage("done");
      setScannedItem({name:"Double-Breasted Wool Blazer",brand:"Zara",category:"Outerwear",color:"#1E2A3A",price:120,tags:["smart","office","evening"],emoji:"🥼",wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",condition:"Like New",forSale:false});
    },5000);
  };
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button className="sb" onClick={async()=>{
                      ...
                      try{
                        ...
                        setScanning(false);
                        setScannedItem({...json,price:finalPrice,wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",forSale:false,sourceImage:productRes.image||null});
                      }catch(e){
                        setScanning(false);
                        setScannedItem({name:"Unknown Item",brand:"Unknown",category:"Tops",color:"#C4A882",price:0,tags:[],emoji:"👚",wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",condition:"Like New",forSale:false});
                      }
                  ...
                  <div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {describeResults.map((r,i)=>(
                        <div key={i} onClick={()=>{
                          setScannedItem({...r,wearCount:0,lastWorn:"Never",purchaseDate:"Mar 2026",forSale:false,sourceImage:r.imageUrl||null});
                          setDescribeResults([]);
                        }}
      ...
      {showReverseSearch&&(
        <ReverseSearchModal
          ...
          onAddToWishlist={(item)=>{
            if(addToWishlist) { addToWishlist({id:Date.now()+Math.random(),emoji:item.emoji,name:item.name,brand:item.brand,price:item.price,gap:"Found via image search",inMarket:item.inMarket,sourceImage:item.sourceImage}); }
            else { setWishlist(prev=>{ if(prev.find(w=>w.name===item.name)) return prev; return [...prev,{id:Date.now()+Math.random(),emoji:item.emoji,name:item.name,brand:item.brand,price:item.price,gap:"Found via image search",inMarket:item.inMarket,sourceImage:item.sourceImage}]; }); }
          }}
        />
      )}
    </div>
...
function CalendarTab({outfits,items,showToast,logWear,events,setEvents}){
  ...
  const addEvent=()=>{
    ...
    const newEv={id:Date.now(),date:newDate,label:newLabel.trim(),occasion:newOccasion,suggestedOutfit:outfits[0]?.id||null,emoji:newEmoji};
...
function ObSlide1(){
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(160deg,#1A140E,#0D0D0D)",padding:"28px",textAlign:"center"}}>
      <div style={{width:88,height:88,borderRadius:24,background:"#C4A88218",border:"1px solid #C4A88244",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:26,boxShadow:"0 0 40px #C4A88222"}}>
        <span style={{fontSize:40,color:"#C4A882"}}>✦</span>
      </div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:300,letterSpacing:5,color:"#F0EBE3",marginBottom:10}}>Outfix</div>
      <div style={{fontFamily:"Montserrat,sans-serif",fontSize:8,fontWeight:600,letterSpacing:3,color:"#5A5048",marginBottom:28}}>YOUR WARDROBE. ELEVATED.</div>
      <div style={{fontFamily:"Montserrat,sans-serif",fontSize:11,color:"#8A7868",lineHeight:1.8,maxWidth:200}}>A private, intelligent home for your wardrobe.</div>
    </div>
  );
}
...
function ObSlide2(){
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0D0D0D",padding:14}}>
      <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,letterSpacing:2,color:"#5A5048",marginBottom:3}}>YOUR CLOSET</div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:300,color:"#F0EBE3",marginBottom:12}}>Build Your Closet</div>
      {[
        ...
        <div key={label} style={{background:"#141414",borderRadius:12,padding:10,border:`1px solid ${gold?"#C4A88244":"#2A2A2A"}`,display:"flex",alignItems:"center",gap:9,marginBottom:6}}>
          <div style={{width:34,height:34,borderRadius:9,background:gold?"#C4A88222":"#1A1A1A",border:`1px solid ${gold?"#C4A88244":"#2A2A2A"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>{ic}</div>
          <div>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:9,fontWeight:600,color:gold?"#C4A882":"#8A7060",letterSpacing:1}}>{label}</div>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#3A3028",marginTop:1}}>{sub}</div>
          </div>
        </div>
      ))}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:4}}>
        {["👚 Tops","👖 Bottoms","🧥 Outerwear","👟 Shoes"].map(item=>(
          <div key={item} style={{background:"#141414",borderRadius:10,padding:8,textAlign:"center",border:"1px solid #2A2A2A",fontFamily:"Montserrat,sans-serif",fontSize:8,color:"#5A5048"}}>{item}</div>
        ))}
      </div>
    </div>
  );
...
function ObSlide3(){
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0D0D0D",padding:14}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:300,color:"#F0EBE3",marginBottom:3}}>Mix & Match</div>
      <div style={{fontFamily:"Montserrat,sans-serif",fontSize:6,letterSpacing:1.5,color:"#5A5048",marginBottom:11}}>SWIPE · HOLD TO REMOVE · TAP TWICE TO LOCK</div>
      {/* Tops — locked */}
      <div style={{marginBottom:9}}>
        <div style={{fontFamily:"Montserrat,sans-serif",fontSize:6,letterSpacing:1,color:"#C4A882",marginBottom:4}}>TOPS 🔒</div>
        <div style={{height:78,borderRadius:14,background:"linear-gradient(135deg,#F5F0E822,#EDE8DF44)",border:"1.5px solid #C4A882",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
          <svg viewBox="0 0 120 110" width="64" height="58" fill="none">
          ...
          <div style={{position:"absolute",bottom:6,right:10,fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#C4A882"}}>Ivory Blouse</div>
        </div>
      </div>
      {/* Bottoms */}
      <div style={{marginBottom:9}}>
        <div style={{fontFamily:"Montserrat,sans-serif",fontSize:6,letterSpacing:1,color:"#5A5048",marginBottom:4}}>BOTTOMS</div>
        <div style={{height:78,borderRadius:14,background:"linear-gradient(135deg,#2C3E5022,#1E304044)",border:"1.5px solid #2A2A2A",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
          <svg viewBox="0 0 120 120" width="60" height="58" fill="none">
          ...
          <div style={{position:"absolute",bottom:6,right:10,fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048"}}>Wide Leg</div>
        </div>
      </div>
      {/* Shoes */}
      <div>
        <div style={{fontFamily:"Montserrat,sans-serif",fontSize:6,letterSpacing:1,color:"#5A5048",marginBottom:4}}>SHOES</div>
        <div style={{height:78,borderRadius:14,background:"linear-gradient(135deg,#1A100822,#2A180A44)",border:"1.5px solid #2A2A2A",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
          <svg viewBox="0 0 140 80" width="90" height="52" fill="none">
          ...
          <div style={{position:"absolute",bottom:6,right:10,fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048"}}>Loafers</div>
        </div>
      </div>
    </div>
  );
...
function ObSlide4(){
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0D0D0D",padding:12,overflow:"hidden"}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:300,color:"#F0EBE3",marginBottom:9}}>Discover</div>
      {posts[0] && (
        <div style={{background:"#141414",borderRadius:12,overflow:"hidden",border:"1px solid #1E1E1E",marginBottom:6}}>
          <div style={{height:52,background:`linear-gradient(135deg,${posts[0].c1}22,${posts[0].c2}33,${posts[0].c3}22)`,display:"flex",alignItems:"flex-end",padding:"0 8px 5px"}}>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:12,color:"#F0EBE3",fontWeight:300}}>{posts[0].outfit}</div>
          </div>
          <div style={{padding:"5px 8px",display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:16,height:16,borderRadius:"50%",background:"#2A2A2A",flexShrink:0}}/>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048",flex:1}}>{posts[0].user} · {posts[0].followers}</div>
            <span style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048"}}>♡ {posts[0].likes}</span>
            <span style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#C4A882",marginLeft:2}}>{posts[0].action}</span>
          </div>
        </div>
      )}
      {/* Trend card */}
      <div style={{borderRadius:11,overflow:"hidden",border:"1px solid #1E1E1E",marginBottom:6}}>
        <div style={{height:20,background:"linear-gradient(135deg,#D4C4A866,#8A786044,#C0B09066)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 8px"}}>
          <div style={{fontFamily:"Montserrat,sans-serif",fontSize:6,fontWeight:600,letterSpacing:1.5,color:"#5A5040",background:"#0D0D0D55",padding:"2px 5px",borderRadius:4}}>TRENDING NOW</div>
          <div style={{display:"flex",gap:3}}>
            {["#D4C4A8","#8A7860","#C0B090","#E8E0D0"].map(c=>(
              <div key={c} style={{width:7,height:7,borderRadius:"50%",background:c,border:"1px solid #0D0D0D44"}}/>
            ))}
          </div>
        </div>
        <div style={{background:"#141414",padding:"7px 9px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:3}}>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:12,color:"#F0EBE3"}}>Quiet Luxury</div>
            <div style={{background:"#0A1A0A",border:"1px solid #1A3A1A",borderRadius:6,padding:"2px 6px",fontFamily:"Montserrat,sans-serif",fontSize:6,color:"#60A870"}}>3 in closet</div>
          </div>
          <div style={{display:"flex",gap:4}}>
            {["neutral","minimal","investment"].map(t=>(
              <div key={t} style={{background:"#1A1A1A",borderRadius:8,padding:"2px 6px",fontFamily:"Montserrat,sans-serif",fontSize:6,color:"#4A4038"}}>{t}</div>
            ))}
          </div>
        </div>
      </div>
      {posts.slice(1).map(p=>(
        <div key={p.user} style={{background:"#141414",borderRadius:12,overflow:"hidden",border:"1px solid #1E1E1E",marginBottom:6}}>
          <div style={{height:44,background:`linear-gradient(135deg,${p.c1}22,${p.c2}33,${p.c3}22)`,display:"flex",alignItems:"flex-end",padding:"0 8px 5px"}}>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:12,color:"#F0EBE3",fontWeight:300}}>{p.outfit}</div>
          </div>
          <div style={{padding:"5px 8px",display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:14,height:14,borderRadius:"50%",background:"#2A2A2A",flexShrink:0}}/>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048",flex:1}}>{p.user} · {p.followers}</div>
            <span style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#5A5048"}}>♡ {p.likes}</span>
            <span style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#C4A882",marginLeft:2}}>{p.action}</span>
          </div>
        </div>
      ))}
    </div>
...
function ObSlide5(){
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0D0D0D",padding:14}}>
      <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,letterSpacing:2,color:"#5A5048",marginBottom:2}}>MEMBERS ONLY</div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:300,color:"#F0EBE3",marginBottom:11}}>The Vault ✦</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:9}}>
        {[["✦","AI Stylist"],["📅","Planner"],["✈️","Vacation"],["📊","Stats"],["🛍","Shoppers"],["📈","Trends"]].map(([ic,l])=>(
          <div key={l} style={{background:"#141414",borderRadius:12,padding:"9px 8px",border:"1px solid #2A2A2A",textAlign:"center"}}>
            <div style={{fontSize:15,marginBottom:3}}>{ic}</div>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:7,color:"#6A5A48"}}>{l}</div>
          </div>
        ))}
      </div>
      <div style={{background:"linear-gradient(135deg,#14100A,#1E1812)",borderRadius:12,padding:10,border:"1px solid #C4A88233",textAlign:"center"}}>
        <div style={{fontFamily:"Montserrat,sans-serif",fontSize:8,color:"#C4A882",letterSpacing:1,marginBottom:6}}>FROM $4 / MONTH</div>
        <div style={{padding:7,borderRadius:8,background:"linear-gradient(135deg,#C4A882,#8A6E54)",fontFamily:"Montserrat,sans-serif",fontSize:8,fontWeight:600,color:"#0D0D0D",letterSpacing:1.5}}>UNLOCK THE VAULT</div>
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:200,background:"#0D0D0D",display:"flex",flexDirection:"column",maxWidth:430,margin:"0 auto",fontFamily:"'Cormorant Garamond','Georgia',serif",color:"#F0EBE3",opacity:exiting?0:1,transition:"opacity 0.4s ease"}}
      ...
      <div style={{display:"flex",justifyContent:"flex-end",padding:"20px 24px 0",flexShrink:0}}>
        {slide<total-1&&<button onClick={finish} style={{background:"none",border:"none",cursor:_p,fontFamily:"Montserrat,sans-serif",fontSize:10,fontWeight:400,color:"#3A3028",letterSpacing:1.5}}>SKIP</button>}
      </div>
      ...
      <div key={slide} style={{flex:1,display:"flex",flexDirection:"column",animation:"obIn 0.35s ease forwards",overflow:"hidden"}}>
        <Screen/>
        {banner&&(
          <div style={{background:"linear-gradient(135deg,#1A1610,#141008)",borderTop:"1px solid #C4A88244",padding:"16px 18px 18px",flexShrink:0}}>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:8,fontWeight:600,letterSpacing:2,color:"#C4A882",marginBottom:6}}>{banner.label}</div>
            <div style={{fontFamily:"Montserrat,sans-serif",fontSize:10,color:"#7A6A58",lineHeight:1.7}}>{banner.body}</div>
          </div>
        )}
      </div>

      {/* Dots + CTA */}
      <div style={{padding:"14px 24px 44px",display:"flex",flexDirection:"column",alignItems:"center",gap:14,flexShrink:0,background:"#0D0D0D"}}>
        <div style={{display:"flex",gap:7}}>
          {Array.from({length:total}).map((_,i)=>(
            <div key={i} onClick={()=>setSlide(i)} style={{width:i===slide?22:6,height:6,borderRadius:3,background:i===slide?"#C4A882":"#2A2418",transition:"all 0.3s ease",cursor:_p}}/>
          ))}
        </div>
        <button onClick={goNext} style={{width:"100%",padding:"15px",borderRadius:16,background:`linear-gradient(135deg,#C4A882,#8A6E54)`,border:"none",fontFamily:"Montserrat,sans-serif",fontSize:10,fontWeight:700,color:"#0D0D0D",letterSpacing:2,cursor:_p,boxShadow:"0 4px 24px #C4A88244"}}>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    (async () => {
    try {
    // Load items, outfits, and wishlist independently so one failure can't break others
    const [itemData, outfitData, wishlistData] = await Promise.all([
      sb.select("items", session.access_token).catch(e=>{ console.error("items load failed:", e); return []; }),
      sb.select("outfits", session.access_token).catch(e=>{ console.error("outfits load failed:", e); return []; }),
      sb.select("wishlist", session.access_token).catch(e=>{ console.error("wishlist load failed:", e); return []; }),
    ]);
  ...
  const logWear = (outfitId) => {
    ...
    setOutfits(prev => prev.map(o => {
      ...
      if(!already) {
        ...
        // Sync outfit worn history to DB
    ...
    setItems(prev => {
      ...
      return prev.map(i => {
        if(!outfit.items.includes(i.id)) return i;
        const newCount = i.wearCount + 1;
        // Sync wear count to DB
        updateWearInDB(i.id, newCount);
        return { ...i, wearCount: newCount, lastWorn: "Today" };
      });
    });
  };
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",overflow:"hidden",transition:"background 0.3s,color 0.3s"}}>
      ...
      <div className="sc" style={{height:"calc(100vh - 130px)",paddingBottom:80}}>
        {tab==="home"     && <HomeTab items={items} outfits={outfits} showToast={showToast} setTab={setTab} setWishlist={setWishlist} addToWishlist={addToWishlist} removeFromWishlist={removeFromWishlist} setItems={setItems} />}
