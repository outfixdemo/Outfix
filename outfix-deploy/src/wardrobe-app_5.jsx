...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  useEffect(()=>{
    // Find the actual scroll container (.sc) — not the feed div
    const getScroller = () => document.querySelector('.sc');
    ...
    const onTouchMove = (e) => {
      ...
      if(!atTop){ setPullProgress(0); pullProgressRef.current=0; return; }
  ...
  return(
    <div ref={feedRef} className="fu" style={{padding:"0 16px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {(pullProgress > 0 || refreshing) && (
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:refreshing?44:`${pullProgress*44}px`,overflow:"hidden",transition:refreshing?"none":"height 0.1s",marginBottom:4}}>
          <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${G}33`,borderTopColor:G,animation:refreshing?"spin 0.7s linear infinite":"none",transform:refreshing?"none":`rotate(${pullProgress*270}deg)`,transition:refreshing?"none":"transform 0.05s"}}/>
        </div>
      )}
