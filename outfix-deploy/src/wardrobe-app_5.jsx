  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        const redeemRes = await fetch("/api/redeem-invite", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ code: inviteCode.trim().toUpperCase() }),
        }).then(r => r.json()).catch(() => ({ valid: false, error: "Could not validate invite code" }));
        ...
        // Mark code as used with the new user's ID
...
// ── HOME ─────────────────────────────────────────────────────────────────────
// ── STORY VIEWER ─────────────────────────────────────────────────────────────
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset,viewProfile,setViewProfile}){
  ...
  const loadLiveFeed = async (offset=0) => {
    ...
    try {
      ...
      const [eventsData, profilesData] = await Promise.all([
        ...
        fetch(`${SB_URL}/rest/v1/profiles?id=in.(${ids})&select=id,username,bio`, {
          headers: {...sbHeaders(session.access_token)}
        }).then(r=>r.json()),
      ]);
      const profileMap = {};
  ...
  const LiveEventCard = ({event}) => {
    ...
    const eventItem = isWore ? {
      ...
      brand: event.item_brand || "",
      category: event.item_category || "",
      sourceImage: event.item_image || null,
      emoji: event.item_emoji || "👗",
      price: 0,
      color: "#C4A882",
      condition: "",
      tags: [],
    ...
    return(
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:20,border:`1px solid ${G}22`}}>
                ...
                {isWore ? (
                  ...
                  (()=>{
                    ...
                    return (event.item_names||[]).slice(0,3).map((name,i)=>{
                      ...
                      const itemObj = { name, brand, category:"", sourceImage:img||null, emoji, price:0, color:sharedColor, condition:"", tags:[] };
                      return(
        ...
        <div
          ...
          <div style={{width:40,height:40,borderRadius:"50%",background:event.isOwn?`${G}33`:"#2A2A2A",border:event.isOwn?`1px solid ${G}55`:"none",overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>
            {event.isOwn ? "✦" : "👤"}
          </div>
          <div style={{flex:1}}>
                  ...
                  <button
                    onClick={async e=>{
                      ...
                      if(session?.access_token){
                        await fetch(`${SB_URL}/rest/v1/feed_events?id=eq.${event.id}`,{
                          method:"DELETE",
    ...
    <div ref={feedRef} className="fu" style={{padding:"0 8px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {liveEvents.length > 0 && (
        <div style={{marginBottom:4}}>
          ...
          <div style={{height:1,background:`${G}22`,marginBottom:20}}/>
          <div style={ss(8,600,DM,{letterSpacing:2,marginBottom:10})}>DISCOVER</div>
        </div>
      )}

      {/* ── Feed: trend cards in Discover ── */}
      {trendItems.slice(0,3).map(trend=>(
        <FeedTrendCard key={trend.id} trend={trend}/>
      ))}
          ...
          {selectedFeedItem._isOutfit ? (
            ...
            <>
              ...
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                {[[selectedFeedItem.price?`$${selectedFeedItem.price}`:"—","PRICE"],[selectedFeedItem.category||"—","CATEGORY"],[selectedFeedItem.condition||"—","CONDITION"]].map(([v,l])=>(
...
function ClosetTab({items,setItems,setSelectedItem,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,onSaveItem}){
  ...
  const [scanCropSrc,setScanCropSrc]=useState(null); // intercepts scan flow for crop first
  ...
  const doScan=async(fileOrDataUrl)=>{
    ...
    try{
          ...
          if(found.length===1){
            ...
            setScannedItem({...found[0],wearCount:0,lastWorn:"Never",purchaseDate:"",forSale:false,stockImage:stockImg||null});
  ...
  // Routes file through CropModal before scanning
  const doScanFile=(file)=>{
    if(!file||!file.type.startsWith("image/")) return;
    const r=new FileReader();
    r.onload=e=>setScanCropSrc(e.target.result);
  ...
  const confirmAdd=()=>{
    ...
    // Prefer stock image over user's photo for a cleaner closet
    const finalImage = scanned.useUserPhoto ? (photoPreview||undefined) : (scanned.stockImage || photoPreview || undefined);
...
- If price unknown, use 0 (do NOT guess a price unless it's in the data above)
...
{"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":${scraped.price||0},"tags":["..."],"emoji":"👚","condition":"Like New"}`;
                        ...
                        // Image: scraped → targeted search with exact product name
                        ...
                        if(!finalImage){
                          ...
                          const queries = [
                            ...
                            `${searchName} ${searchBrand} clothing`,
                            `${detectedProductSlug} ${searchBrand}`,
                          ].filter(Boolean);
                        ...
                        setScanning(false);
                        setScannedItem({...json,price:finalPrice,wearCount:0,lastWorn:"Never",purchaseDate:"",forSale:false,sourceImage:finalImage});
                      }catch(e){
                        setScanning(false);
              ...
              <div style={{marginTop:16}}>
                {(()=>{
                  ...
                  const confirmManual=()=>{
                    ...
                    setScannedItem({name:mName.trim(),brand:mBrand.trim()||"Unknown",category:mCat,color:mColor,colors:mColors,price:parseInt(mPrice)||0,tags:[],emoji,condition:mCondition,wearCount:0,lastWorn:"Never",purchaseDate:mDate,forSale:false,sourceImage:photoPreview||null});
                  };
                  return(
                    <>
            ...
            {manualCropSrc&&(
              <CropModal
                src={manualCropSrc}
                onCancel={()=>setManualCropSrc(null)}
                onSave={cropped=>{setPhotoPreview(cropped);setManualCropSrc(null);}}
                autoRemoveBg={true}
              />
            )}
            {scanCropSrc&&(
              <CropModal
                src={scanCropSrc}
                onCancel={()=>setScanCropSrc(null)}
                onSave={cropped=>{setScanCropSrc(null);doScan(cropped);}}
                autoRemoveBg={true}
              />
            )}
            {scanning && (
            ...
            {scanned && (
              <div style={{marginTop:addMode==="describe"?20:0}}>
                ...
                <div style={{display:"flex",gap:14,marginBottom:16}}>
                  {/* Image box — stock photo by default, toggle to user photo */}
                  <div style={{position:"relative",width:72,height:72,flexShrink:0}}>
                    <input ref={photoOverrideRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
                      const file=e.target.files?.[0]; if(!file) return;
                      const reader=new FileReader();
                      reader.onload=ev=>{ setPhotoPreview(ev.target.result); setScannedItem(prev=>({...prev,useUserPhoto:true})); };
                    ...
                    <div style={{width:72,height:72,borderRadius:14,background:`linear-gradient(135deg,${scanned.color}22,${scanned.color}55)`,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",border:_2a}}>
                      {/* Show stock image first, fall back to user photo, then illustration */}
  ...
  return new Promise(resolve=>{
    ...
    img.onload=()=>{
      ...
      for(let start=0;start<w*h;start++){
        ...
        if(blobPixels.length>largestSize){largestSize=blobPixels.length;largestBlob=blobPixels;}
      }
      const keep=new Uint8Array(w*h);
      for(const i of largestBlob) keep[i]=1;
...
function CropModal({src, onCancel, onSave, autoRemoveBg=false}){
  ...
  const [zoneH,setZoneH]=useState(0); // measured height of image zone
  const containerRef=useRef();
  const imgRef=useRef();
  const zoneRef=useRef(); // ref on the image zone div to measure actual height

  // Measure the image zone height after layout
  useEffect(()=>{
  ...
  useEffect(()=>{
    if(zoneH===0) return; // wait until zone is measured
    const img=new Image();
    img.onload=()=>{
      const nat={w:img.naturalWidth,h:img.naturalHeight};
      setImgNatural(nat);
      const maxW=Math.min(window.innerWidth-40,390);
      const maxH=zoneH; // use actual measured zone height
  ...
  const applyWithBgRemoval=async()=>{
    ...
    try{
      ...
      const spillBlob=await removeWhiteSpill(resultBlob);
      const finalBlob=await cleanIsolatedPixels(spillBlob);
      const reader=new FileReader();
  ...
  return(
    <div style={{..._fix,inset:0,background:"#000000EE",zIndex:200,display:"flex",flexDirection:"column",padding:"12px 20px 20px",paddingBottom:"calc(20px + env(safe-area-inset-bottom, 0px) + 70px)",boxSizing:"border-box"}}
      onTouchMove={e=>e.preventDefault()}
    >
      {/* Header */}
      <div style={{..._btwn,marginBottom:8,flexShrink:0}}>
        <div style={sr(18,400)}>Crop Photo</div>
        <div style={ss(9,400,DM)}>Drag · Pinch · Corner handles</div>
      </div>

      {/* Image fills remaining space — zoneRef measures actual available height */}
      <div ref={zoneRef} style={{flex:1,minHeight:0,width:"100%",display:"flex",alignItems:"center",justifyContent:"center",marginTop:4,marginBottom:4,overflow:"hidden"}}>
        <div ref={containerRef} style={{position:"relative",width:displaySize.w,height:displaySize.h,maxWidth:"100%",maxHeight:"100%",cursor:"move",touchAction:"none",flexShrink:0}}
        ...
        >
          <img ref={imgRef} src={src} style={{width:displaySize.w,height:displaySize.h,objectFit:"contain",display:"block",userSelect:"none",pointerEvents:"none",borderRadius:10}} alt="crop"/>
      ...
      <div style={{flexShrink:0,marginTop:8}}>
        <div style={ss(9,400,DM,{letterSpacing:1,marginBottom:5,textAlign:"center"})}>DRAG BOX · PINCH OR SLIDE TO RESIZE · DRAG CORNERS</div>
        ...
        />
        {bgError&&(
          <div style={{marginBottom:8,padding:"8px 14px",borderRadius:10,background:"#2A1A1A",border:"1px solid #CC333344",...ss(10,500,"#CC6666",{textAlign:"center"})}}>
            {bgError}
          </div>
        )}
        <div style={{display:"flex",gap:8}}>
          <button onClick={onCancel} disabled={removingBg} style={{flex:1,padding:"13px",borderRadius:14,background:"#1A1A1A",border:"1px solid #2A2A2A",...ss(10,600,DM,{letterSpacing:1}),cursor:_p,opacity:removingBg?0.5:1}}>CANCEL</button>
          {autoRemoveBg ? (
            <button onClick={applyWithBgRemoval} disabled={removingBg} style={{flex:3,padding:"13px",borderRadius:14,background:removingBg?"#2A2A2A":`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(10,700,removingBg?DM:BK,{letterSpacing:1}),cursor:_p,transition:"all 0.3s"}}>
              {removingBg?"REMOVING BG…":"✦ CROP & REMOVE BG"}
            </button>
          ) : (
            <>
              <button onClick={applyCrop} disabled={removingBg} style={{flex:1.4,padding:"13px",borderRadius:14,background:"#1A1A1A",border:`1px solid ${G}`,...ss(10,600,G,{letterSpacing:1}),cursor:_p,opacity:removingBg?0.5:1}}>CROP</button>
              <button onClick={applyWithBgRemoval} disabled={removingBg} style={{flex:2,padding:"13px",borderRadius:14,background:removingBg?"#2A2A2A":`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(10,700,removingBg?DM:BK,{letterSpacing:1}),cursor:_p,transition:"all 0.3s"}}>
...
// ── MIX & MATCH BUILDER ──────────────────────────────────────────────────────
function MixMatchBuilder({tops,bottoms,shoes,outerwear,accessories,showToast,logWear,outfits,setOutfits,setItems,items,onNewLook,onSaveOutfit,styleProfile={},saveStyleProfile,postWearFeedEvent}){
      ...
      {lastAiCombo && !aiLoading && (
        <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center",justifyContent:"center"}}>
          ...
          <button onClick={()=>{
            ...
            setLastAiCombo(null);
            showToast("Combo liked — AI will remember \u2746");
          }} style={{padding:"6px 14px",borderRadius:20,background:"#1A2A1A",border:"1px solid #2A4A2A",cursor:_p,...ss(13,400,"#80C880"),display:"flex",alignItems:"center",gap:4}}>
            👍
          </button>
          <button onClick={()=>{
            const combo = [top,isDress?null:bottom,shoe,showOuterwear?outer:null,showAccessories?accessory:null].filter(Boolean);
            ...
            setLastAiCombo(null);
            showToast("Combo noted — AI will avoid similar \u2746");
          }} style={{padding:"6px 14px",borderRadius:20,background:"#2A1A1A",border:"1px solid #4A2A2A",cursor:_p,...ss(13,400,"#C08080"),display:"flex",alignItems:"center",gap:4}}>
            👎
          </button>
        </div>
      )}
...
function OutfitsTab({items,outfits,setOutfits,setItems,showToast,logWear,onSaveOutfit,onDeleteOutfit,styleProfile={},saveStyleProfile,postWearFeedEvent}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        // Build posts from outfits — outfits.items is an array of item IDs (may be numeric or UUID)
        const posts = Array.isArray(outfits) ? outfits.map(o=>{
          const outfitItemIds = Array.isArray(o.items) ? o.items : [];
        ...
        setRealProfile({
          ...
          name: resolvedProf?.username || username || "Outfix User",
    ...
    <div style={{..._fix,background:BK,zIndex:400,maxWidth:430,margin:"0 auto",display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      ...
      <div style={{padding:"14px 18px 0",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:0,marginBottom:16,background:"#111",borderRadius:14,overflow:"hidden",border:"1px solid #1E1E1E"}}>
          {[
            {label:"Followers", value:profile.followers, type:"followers"},
            {label:"Following", value:profile.following, type:"following"},
          ].map((s,i)=>(
            <div key={i}
              onClick={()=>loadFollowList(s.type)}
              onTouchEnd={e=>{e.stopPropagation();loadFollowList(s.type);}}
              style={{flex:1,padding:"12px 8px",textAlign:"center",borderRight:i<1?"1px solid #1E1E1E":"none",cursor:_p}}>
      ...
      {activeTab==="posts"&&(
        <div style={{padding:"16px 18px",flex:1}}>
          ...
          {(profile.recentPosts||[]).map((post,pi)=>{
            ...
            const postEmojis = postItems.slice(0,4).map(i=>i?.emoji).filter(Boolean);
            if(!postEmojis.length) postEmojis.push("👗","👟","✨");
            const postAccent = POST_ACCENTS[pi%POST_ACCENTS.length];
            return(
              <div key={post.id||pi} style={{background:"#111",borderRadius:18,overflow:"hidden",marginBottom:14,border:"1px solid #1E1E1E"}}>
                <div style={{width:"100%",position:"relative"}}>
                  <div style={{width:"100%",paddingTop:"56%",position:"relative",overflow:"hidden"}}>
                    <div style={{..._abs0,background:`linear-gradient(135deg,${postAccent}18,${postAccent}38)`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      <div style={{display:"flex",gap:14,fontSize:40}}>{postEmojis.map((e,i)=><span key={i}>{e}</span>)}</div>
                    </div>
                  </div>
        ...
        <div style={{padding:"16px 18px",flex:1}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {(profile.allItems||[]).map((item,i)=>{
              ...
              return(
              <div key={i} onClick={()=>setSelectedProfileItem({...item,_idx:i})} style={{background:"#111",borderRadius:14,overflow:"hidden",border:"1px solid #1E1E1E",cursor:_p}}>
      ...
      {selectedProfileItem&&(
        <div onClick={()=>setSelectedProfileItem(null)} style={{position:"fixed",top:76,left:0,right:0,bottom:0,background:"#000C",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 16px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:24,width:"100%",maxWidth:430,maxHeight:"88vh",overflowY:"auto",border:`1px solid ${G}22`}}>
            {/* Close button */}
            <div style={{display:"flex",justifyContent:"flex-end",padding:"12px 16px 0"}}>
              <button onClick={()=>setSelectedProfileItem(null)} style={{width:28,height:28,borderRadius:"50%",background:"#1A1A1A",border:"1px solid #2A2A2A",cursor:_p,...ss(13,400,DM),display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
            {/* Image */}
            <div style={{width:"100%",height:260,background:`linear-gradient(135deg,${selectedProfileItem.color||"#2A2A2A"}18,${selectedProfileItem.color||"#2A2A2A"}40)`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
              {selectedProfileItem.sourceImage||selectedProfileItem.source_image
                ? <img src={selectedProfileItem.sourceImage||selectedProfileItem.source_image} style={{width:"100%",height:"100%",objectFit:"contain",padding:16,boxSizing:"border-box"}} alt={selectedProfileItem.name}/>
                : <ItemIllustration item={selectedProfileItem} size={140}/>
              }
            </div>
            {/* Info */}
            <div style={{padding:"16px 20px 40px"}}>
              <div style={{..._btwn,marginBottom:4}}>
                <div style={sr(22,500)}>{selectedProfileItem.name}</div>
                {(selectedProfileItem.forSale||selectedProfileItem.for_sale)&&<div style={{background:G,borderRadius:8,padding:"3px 10px",...ss(8,700,BK,{letterSpacing:1})}}>FOR SALE</div>}
              </div>
              <div style={{..._row,gap:8,marginBottom:16}}>
                <div style={{width:10,height:10,borderRadius:"50%",background:selectedProfileItem.color||G,border:"1px solid #FFFFFF22"}}/>
                <div style={ss(10,400,DM)}>{selectedProfileItem.brand}</div>
                {selectedProfileItem.category&&<div style={ss(10,400,DM)}>· {selectedProfileItem.category}</div>}
                {selectedProfileItem.condition&&<div style={ss(10,400,DM)}>· {selectedProfileItem.condition}</div>}
              </div>
              {/* Stats row */}
              <div style={{display:"flex",gap:10,marginBottom:20}}>
                {[[`$${selectedProfileItem.price||0}`,"VALUE"],[selectedProfileItem.wearCount!=null?`${selectedProfileItem.wear_count||selectedProfileItem.wearCount||0}×`:"—","WORN"],[selectedProfileItem.category||"—","CATEGORY"]].map(([v,l])=>(
                  <div key={l} style={{flex:1,background:"#111",borderRadius:12,padding:"10px",textAlign:"center",border:"1px solid #1E1E1E"}}>
                    <div style={sr(15,500,G)}>{v}</div>
                    <div style={ss(7,400,DM,{letterSpacing:1,marginTop:2})}>{l}</div>
                  </div>
                ))}
              ...
              {/* Tags */}
              {(selectedProfileItem.tags||[]).length>0&&(
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
                  {selectedProfileItem.tags.map(t=><div key={t} style={{background:"#1A1A1A",borderRadius:8,padding:"3px 10px",border:"1px solid #2A2A2A",...ss(9,400,DM)}}>{t}</div>)}
                </div>
              )}
              {/* Actions */}
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{
                  ...
                  const wishItem = {id:Date.now(),name:item.name,brand:item.brand||"Unknown",emoji:item.emoji||"👗",price:item.price||0,gap:"From @"+(item._username||"user"),inMarket:item.forSale||item.for_sale||false,sourceImage:item.sourceImage||item.source_image||null,color:item.color||null};
                  if(addToWishlist) addToWishlist(wishItem);
                  showToast(`${item.name} added to wishlist \u2746`);
                  setSelectedProfileItem(null);
                }} style={{flex:1,padding:"14px",borderRadius:16,background:"#1A1A2A",border:`1px solid ${G}44`,...ss(10,600,G,{letterSpacing:1}),cursor:_p}}>
                  ♡ WISHLIST
                </button>
                {onAddToCloset&&(
                  <button onClick={async()=>{
                    const idx = selectedProfileItem._idx;
                    if(addedItems.has(selectedProfileItem.id||idx)) return;
                    const newItem={id:Date.now(),name:selectedProfileItem.name,brand:selectedProfileItem.brand||"Unknown",category:selectedProfileItem.category||"Tops",color:selectedProfileItem.color||"#C4A882",price:selectedProfileItem.price||0,emoji:selectedProfileItem.emoji||"👗",wearCount:0,lastWorn:"Never",purchaseDate:"",condition:selectedProfileItem.condition||"Good",forSale:false,tags:[],sourceImage:selectedProfileItem.sourceImage||selectedProfileItem.source_image||null};
                    await onAddToCloset(newItem);
                    setAddedItems(prev=>new Set([...prev,selectedProfileItem.id||idx]));
                    showToast(`${selectedProfileItem.name} added to your closet \u2746`);
                    setSelectedProfileItem(null);
                  }} style={{flex:1,padding:"14px",borderRadius:16,background:addedItems.has(selectedProfileItem.id||selectedProfileItem._idx)?"#1A2A1A":`linear-gradient(135deg,${G},#8A6E54)`,border:addedItems.has(selectedProfileItem.id||selectedProfileItem._idx)?"1px solid #2A4A2A":"none",...ss(10,700,addedItems.has(selectedProfileItem.id||selectedProfileItem._idx)?"#80C880":BK,{letterSpacing:1.5}),cursor:_p}}>
                    {addedItems.has(selectedProfileItem.id||selectedProfileItem._idx)?"✓ ADDED":"+ CLOSET"}
                  </button>
                )}
              </div>
            </div>
...
Respond ONLY with JSON: {"days":[{"day":1,"date":"${tripData.startDate}","label":"Arrival & Check-in","activity":"Travel","emoji":"✈️"},...]}`
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      <div onTouchStart={handleVacSwipeStart} onTouchEnd={handleVacSwipeEnd}
        ...
        {weather&&!wxLoading&&(
          <div style={{marginTop:14}}>
            ...
            {/* Seasonal summary text — shown for far-out trips */}
            {weather.seasonal&&weather.summary&&(
              <div style={{...ss(9,400,"#7A90B8",{lineHeight:1.5,fontStyle:"italic",marginBottom:10})}}>{weather.summary}</div>
            )}
            {/* Day cards — shown for both modes */}
              ...
              {weather.daily.map(d=>(
                <div key={d.date} style={{flexShrink:0,textAlign:"center",background:"#0D1928",borderRadius:12,padding:"8px 10px",border:"1px solid #1A2A40",minWidth:48}}>
                  ...
                  <div style={ss(7,400,"#3A5070",{marginTop:3})}>
                    {["Su","Mo","Tu","We","Th","Fr","Sa"][new Date(d.date+"T12:00:00").getDay()]}
                  </div>
                </div>
              ))}
            </div>
                ...
                {isOpen&&(
                  <div style={{marginTop:14,borderTop:`1px solid ${BR}`,paddingTop:14}}>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
                      ...
                      <button onClick={e=>{e.stopPropagation();setShowOutfitPicker(di);setOutfitSearch("");}} style={{width:52,height:52,borderRadius:14,background:"#0F0F0F",border:"1.5px dashed #2A2A2A",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,color:"#3A3028",cursor:_p}}>+</button>
                    </div>
      ...
      {confirmDeleteTrip&&(
        <div onClick={()=>setConfirmDeleteTrip(false)} style={{..._fix,inset:0,background:"#000000BB",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,margin:"0 auto",padding:"24px 24px 44px",border:"1px solid #2A2A2A"}}>
            <div style={{width:36,height:4,borderRadius:2,background:"#333",margin:"0 auto 20px"}}/>
            <div style={{textAlign:"center",marginBottom:20}}>
              <div style={{fontSize:36,marginBottom:12}}>🗑</div>
              <div style={sr(20,400,undefined,{marginBottom:8})}>Delete "{vacation.name}"?</div>
              <div style={ss(11,400,DM,{lineHeight:1.5})}>This will permanently remove the trip, all itinerary days, and outfit assignments. This cannot be undone.</div>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setConfirmDeleteTrip(false)} style={{flex:1,padding:"14px",borderRadius:14,background:_1a,border:_2a,...ss(10,600,DM,{letterSpacing:1}),cursor:_p}}>KEEP TRIP</button>
              <button onClick={async()=>{
              ...
              }} style={{flex:1,padding:"14px",borderRadius:14,background:"#2A0A0A",border:"1px solid #5A1A1A",...ss(10,700,"#E08080",{letterSpacing:1}),cursor:_p}}>DELETE</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* EDIT TRIP SHEET */}
      {editingTrip&&(
        <div onClick={()=>setEditingTrip(false)} style={{..._fix,inset:0,background:"#000000BB",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,margin:"0 auto",padding:"20px 24px 40px",border:"1px solid #2A2A2A",maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{width:36,height:4,borderRadius:2,background:"#333",margin:"0 auto 20px"}}/>
            <div style={sr(20,400,undefined,{marginBottom:20})}>Edit Trip Details</div>
            <div style={{..._col,gap:12}}>
              <div>
                <div style={ss(9,400,DM,{letterSpacing:1.5,marginBottom:6})}>TRIP NAME</div>
                <input value={editTripForm.name} onChange={e=>setEditTripForm(p=>({...p,name:e.target.value}))}
                  style={{width:"100%",boxSizing:"border-box",background:_1a,border:_2a,borderRadius:12,padding:"11px 14px",...ss(13,400,MD),color:"#C0B8B0",outline:"none"}}/>
              </div>
              <div>
                <div style={ss(9,400,DM,{letterSpacing:1.5,marginBottom:6})}>DESTINATION</div>
                <input value={editTripForm.destination} onChange={e=>setEditTripForm(p=>({...p,destination:e.target.value}))}
                  style={{width:"100%",boxSizing:"border-box",background:_1a,border:_2a,borderRadius:12,padding:"11px 14px",...ss(13,400,MD),color:"#C0B8B0",outline:"none"}}/>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <FullDatePicker label="Start Date" value={editTripForm.startDate} onChange={v=>setEditTripForm(p=>({...p,startDate:v}))}/>
                <FullDatePicker label="End Date" value={editTripForm.endDate} onChange={v=>setEditTripForm(p=>({...p,endDate:v}))}/>
              </div>
              <div>
                <div style={ss(9,400,DM,{letterSpacing:1.5,marginBottom:8})}>CLIMATE</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {["Warm & Sunny","Cold & Snowy","Tropical & Humid","Mediterranean","Rainy & Cool"].map(c=>(
                    <button key={c} onClick={()=>setEditTripForm(p=>({...p,climate:c}))} style={{padding:"6px 12px",borderRadius:20,background:editTripForm.climate===c?G:"#1A1A1A",border:editTripForm.climate===c?"none":"1px solid #222",...ss(9,editTripForm.climate===c?600:400,editTripForm.climate===c?BK:DM,{letterSpacing:0.5}),cursor:_p}}>{c}</button>
                  ))}
                </div>
              </div>
              <div style={{display:"flex",gap:10,marginTop:8}}>
                <button onClick={()=>setEditingTrip(false)} style={{flex:1,padding:"13px",borderRadius:14,background:_1a,border:_2a,...ss(10,600,DM,{letterSpacing:1}),cursor:_p}}>CANCEL</button>
                <button onClick={async()=>{
                ...
                }} style={{flex:2,padding:"13px",borderRadius:14,background:`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(10,700,BK,{letterSpacing:1.5}),cursor:_p}}>SAVE CHANGES</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showOutfitPicker!==null&&(
        <div onClick={()=>setShowOutfitPicker(null)} style={{..._fix,inset:0,background:"#000000BB",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,margin:"0 auto",padding:"20px 20px 40px",maxHeight:"70vh",overflowY:"auto",border:`1px solid #2A2A2A`}}>
            <div style={{width:36,height:4,borderRadius:2,background:"#333",margin:"0 auto 16px"}}/>
            <div style={sr(18,400,undefined,{marginBottom:14})}>Add items to Day {vacation.days_plan[showOutfitPicker]?.day}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {items.filter(it=>{
                ...
                const alreadyAdded=(vacation.days_plan[showOutfitPicker]?.outfitIds||[]).includes(it.id);
                return(
                  <div key={it.id} onClick={()=>{
                    if(alreadyAdded) return;
                    const updated={...vacation,days_plan:vacation.days_plan.map((d,i)=>i===showOutfitPicker?{...d,outfitIds:[...(d.outfitIds||[]),it.id]}:d)};
                    updateVacation(updated);
                    showToast(`${it.name} added ✦`);
                  }} style={{background:alreadyAdded?`${G}18`:"#111",borderRadius:12,overflow:"hidden",border:alreadyAdded?`1.5px solid ${G}44`:"1px solid #2A2A2A",cursor:alreadyAdded?"default":_p,opacity:alreadyAdded?0.7:1}}>
                    <div style={{height:80,background:`${it.color||"#2A2A2A"}18`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                      ...
                      {alreadyAdded&&<div style={{position:"absolute",inset:0,background:"#00000066",display:"flex",alignItems:"center",justifyContent:"center",...ss(18,600,G)}}>✓</div>}
                    </div>
...
function CalendarTab({outfits,items,showToast,logWear,events,setEvents,session}){
  ...
  const [thumbsFeedback,setThumbsFeedback]=useState({}); // outfitId -> 'up'|'down'
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      {/* VACATION VIEW */}
      {view==="vacation"&&(
        <VacationPlanner items={items} outfits={outfits} showToast={showToast} onBack={()=>setView("vacation")} session={session}/>
      )}
                  ...
                  {aiOutfits.map(o=>{
                    ...
                    return(
                      <div key={o.id} style={{background:CD,borderRadius:16,padding:"14px",border:`1px solid ${BR}`,marginBottom:10}}>
                        ...
                        <div style={{display:"flex",gap:8,marginBottom:8}}>
                          <button onClick={()=>setThumbsFeedback(p=>({...p,[o.id]:p[o.id]==="up"?null:"up"}))} style={{padding:"6px 14px",borderRadius:20,background:thumbsFeedback[o.id]==="up"?"#1A2A1A":"#111",border:thumbsFeedback[o.id]==="up"?"1px solid #2A4A2A":"1px solid #2A2A2A",cursor:_p,...ss(13,400,thumbsFeedback[o.id]==="up"?"#80C880":DM)}}>👍</button>
                          <button onClick={()=>setThumbsFeedback(p=>({...p,[o.id]:p[o.id]==="down"?null:"down"}))} style={{padding:"6px 14px",borderRadius:20,background:thumbsFeedback[o.id]==="down"?"#2A1A1A":"#111",border:thumbsFeedback[o.id]==="down"?"1px solid #4A2A2A":"1px solid #2A2A2A",cursor:_p,...ss(13,400,thumbsFeedback[o.id]==="down"?"#C08080":DM)}}>👎</button>
                          {thumbsFeedback[o.id]&&<div style={ss(9,400,DM,{alignSelf:"center",fontStyle:"italic"})}>{thumbsFeedback[o.id]==="up"?"Noted ✦":"Noted — will try a different style"}</div>}
                        </div>
...
function DuplicatesSection({items,showToast}){
  ...
  // Build real duplicate groups by comparing items by category + color similarity
  const duplicateGroups = useMemo(()=>{
    ...
    const colorClose=(a,b)=>{
      ...
      return diff<0x202020; // similar hue/brightness
    };
    items.forEach((itemA,i)=>{
      if(used.has(itemA.id)) return;
      const matches=items.slice(i+1).filter(itemB=>{
        if(used.has(itemB.id)) return false;
        return itemA.category===itemB.category && colorClose(itemA.color,itemB.color);
      });
      if(matches.length>0){
        const group={id:`g${itemA.id}`,label:`Similar ${itemA.category.toLowerCase()}`,items:[itemA,...matches],similarity:78,
          reason:`These ${itemA.category.toLowerCase()} share a similar color and category. Consider which works hardest for your lifestyle.`};
        groups.push(group);
        matches.forEach(m=>used.add(m.id));
        used.add(itemA.id);
      }
    });
    return groups.filter(g=>!dismissed.has(g.id));
  },[items,dismissed]);
...
function PushNotifPreview({onClose,showToast,session,notifs,setNotifs,notifsLoaded,setNotifsLoaded}){
  ...
  const loadRealNotifs = async () => {
    ...
    try {
      ...
      // Build like notifications from events with like_count > 0
      const likeNotifs = (Array.isArray(myEvents)?myEvents:[])
        .filter(e=>(e.like_count||0)>0)
        .map(e=>({
          id:`like-${e.id}`, type:"like", read:false, icon:"♥",
          title:`Your post got ${e.like_count} like${e.like_count>1?"s":""}`,
          body:`"${e.outfit_name||e.item_name||"post"}"`,
          time: getTimeAgo(e.created_at), urgent:false,
        }));

      // Build follow notifications
      const followNotifs = (Array.isArray(followers)?followers:[]).slice(0,5).map(f=>({
        id:`follow-${f.follower_id}`, type:"follow", read:false, icon:"👤",
        title:`@${followerProfiles[f.follower_id]?.username||"Someone"} followed you`,
        body:"Check out their closet",
        time: getTimeAgo(f.created_at), urgent:false,
      }));

      const allNotifs = [...likeNotifs, ...followNotifs, ...suggestNotifs]
        .sort((a,b)=> a.time==="Suggested"?1:-1);
...
function SettingsTab({currentPlan,setShowPricing,showToast,items,outfits=[],userName="",userEmail="",onSignOut,userProfile={},saveProfile,styleProfile={},saveStyleProfile,onViewOwnProfile}){
  ...
  return(
    <div className="fu" style={{padding:"16px 24px"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1A1A0A,#2A2010)",borderRadius:20,padding:"20px",marginBottom:20,border:`1px solid ${G}33`,textAlign:"center"}}>
        <div onClick={onViewOwnProfile} style={{width:72,height:72,borderRadius:"50%",background:`linear-gradient(135deg,${G},#8A6E54)`,margin:"0 auto 12px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,cursor:onViewOwnProfile?_p:"default"}}>✦</div>
...
// ── VAULT ────────────────────────────────────────────────────────────────────
function VaultTab({items,outfits,showToast,wishlist,setWishlist,addToWishlist,removeFromWishlist,currentPlan,setShowPricing,logWear,events,setEvents,session}){
...
export default function App(){
  ...
  const logWear = (outfitId) => {
    ...
    // Find outfit synchronously before any state updates
    const wornOutfit = outfits.find(o => o.id === outfitId);
    ...
    // Post feed event — wornOutfit is guaranteed non-null here
    if(session?.access_token && wornOutfit) {
      const uid = session.user?.id;
      const outfitItemObjs = outfitItemIds.map(id=>items.find(i=>i.id===id)).filter(Boolean);
  ...
  return(
    <div style={{fontFamily:"'Cormorant Garamond','Georgia',serif",background:wrapBg,minHeight:"100vh",color:wrapColor,maxWidth:430,margin:"0 auto",position:"relative",transition:"background 0.3s,color 0.3s"}}>
      ...
      <div onClick={()=>document.getElementById('main-scroll')?.scrollTo({top:0,behavior:'smooth'})}
        ...
        <div style={{..._btwn}}>
          ...
          <div style={{..._row,gap:10}}>
            ...
            <button className="tb" onClick={()=>setTab("__settings")} style={{width:38,height:38,borderRadius:"50%",border:`1px solid ${divLine}`,display:"flex",alignItems:"center",justifyContent:"center",background:"none",cursor:_p}}>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <circle cx="10" cy="10" r="3" stroke={G} strokeWidth="1.4" fill="none"/>
                <path d="M10 2V4M10 16V18M2 10H4M16 10H18M4.2 4.2L5.6 5.6M14.4 14.4L15.8 15.8M4.2 15.8L5.6 14.4M14.4 5.6L15.8 4.2" stroke={G} strokeWidth="1.4" strokeLinecap="round"/>
              </svg>
            </button>
          </div>
        </div>
      ...
      <div id="main-scroll" className="sc" style={{height:"100vh",paddingTop:76,paddingBottom:80,boxSizing:"border-box"}}>
        {tab==="home"     && <HomeTab items={items} outfits={outfits} showToast={showToast} setTab={setTab} setWishlist={setWishlist} addToWishlist={addToWishlist} removeFromWishlist={removeFromWishlist} setItems={setItems} session={session} viewProfile={viewProfile} setViewProfile={setViewProfile} onAddToCloset={async(item)=>{
        ...
        {tab==="outfits"   && <OutfitsTab items={items} outfits={outfits} setOutfits={setOutfits} setItems={setItems} showToast={showToast} logWear={logWear} onSaveOutfit={saveOutfitToDB} onDeleteOutfit={deleteOutfitFromDB} styleProfile={styleProfile} saveStyleProfile={saveStyleProfile} postWearFeedEvent={postWearFeedEvent}/>}
        {tab==="market"    && (
        ...
        {tab==="vault"     && <VaultTab items={items} outfits={outfits} showToast={showToast} wishlist={wishlist} setWishlist={setWishlist} addToWishlist={addToWishlist} removeFromWishlist={removeFromWishlist} currentPlan={currentPlan} setShowPricing={setShowPricing} logWear={logWear} events={appEvents} setEvents={setAppEvents} session={session}/>}
      </div>
