...
function CropModal({src, onCancel, onSave, autoRemoveBg=false}){
  ...
  useEffect(()=>{
    ...
    img.onload=()=>{
      ...
      const maxH=Math.round(window.innerHeight*0.48); // leave room for header+slider+buttons
  ...
  return(
    <div style={{..._fix,inset:0,background:"#000000EE",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",padding:"16px 24px",overflowY:"auto"}}>
      <div style={{width:"100%",maxWidth:430,paddingBottom:16}}>
        {/* Header */}
        ...
        {/* Image + crop overlay */}
        <div ref={containerRef} style={{position:"relative",width:displaySize.w,height:displaySize.h,margin:"0 auto",borderRadius:12,overflow:"visible",cursor:"move",touchAction:"none"}}
        ...
        >
          ...
          {/* Dark overlay + crop box — SVG extends beyond image so box renders outside */}
          <svg style={{position:"absolute",left:-(cropSize),top:-(cropSize),pointerEvents:"none",overflow:"visible"}}
            ...
            {/* Corner handles — large touch targets for resize drag */}
            {[[0,0],[1,0],[0,1],[1,1]].map(([hx,hy])=>{
              ...
              return(
                <g key={`${hx}${hy}`}
                  ...
                  <rect x={cx2-22} y={cy2-22} width={44} height={44} fill="transparent"/>
                  {/* Visible L-shaped corner */}
        ...
        {/* BG removal error */}
        {bgError&&(
          <div style={{marginBottom:10,padding:"10px 14px",borderRadius:10,background:"#2A1A1A",border:"1px solid #CC333344",...ss(10,500,"#CC6666",{textAlign:"center"})}}>
            {bgError}
          </div>
        )}
        {/* Actions */}
        <div style={{display:"flex",gap:8}}>
          ...
          {autoRemoveBg ? (
            // New item flow — single button that crops + removes bg automatically
            ...
            // Existing item edit flow — keep separate crop and remove bg buttons
            <>
