  ...
  useEffect(()=>{
    ...
    const onTouchMove = (e) => {
      ...
      if(!scroller || scroller.scrollTop > 2) {
        if(pullProgressRef.current > 0){ setPullProgress(0); pullProgressRef.current=0; }
        return;
      }
      const dy = e.touches[0].clientY - touchStartY.current;
      if(dy <= 0){ setPullProgress(0); pullProgressRef.current=0; return; }
    ...
    const onTouchEnd = () => {
      if(pullProgressRef.current >= 1 && !refreshingRef.current){
        refreshingRef.current = true;
        setRefreshing(true);
        setPullProgress(0);
        pullProgressRef.current = 0;
        loadLiveFeed().finally(()=>{ setRefreshing(false); refreshingRef.current=false; });
      } else {
        setPullProgress(0);
        pullProgressRef.current = 0;
      }
...
function UserProfilePage({ handle, userId, username, onClose, showToast, session, onAddToCloset }) {
  ...
  return (
    <div style={{..._fix,background:BK,zIndex:400,maxWidth:430,margin:"0 auto",display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      ...
      {/* ── ITEM DETAIL SHEET ── */}
      {selectedProfileItem&&(
        <div onClick={()=>setSelectedProfileItem(null)} style={{..._fix,background:"#000C",zIndex:500,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,margin:"0 auto",maxHeight:"88vh",overflowY:"auto",border:`1px solid ${G}22`}}>
            {/* Handle */}
