      ...
      {selectedFeedItem&&(
        <div onClick={()=>setSelectedFeedItem(null)} style={{position:"fixed",top:76,left:0,right:0,bottom:0,background:"#000C",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 16px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0D0D0D",borderRadius:24,width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${G}22`}}>
            ...
            <div style={{width:"100%",height:240,background:`linear-gradient(135deg,${selectedFeedItem.color||"#2A2A2A"}18,${selectedFeedItem.color||"#2A2A2A"}40)`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
              {selectedFeedItem.sourceImage
                ...
                : <ItemIllustration item={selectedFeedItem} size={130}/>
              }
            </div>
            <div style={{padding:"16px 20px 36px"}}>
              <div style={{..._btwn,marginBottom:4}}>
                <div style={sr(22,500)}>{selectedFeedItem.name}</div>
                {selectedFeedItem.forSale&&<div style={{background:G,borderRadius:8,padding:"3px 10px",...ss(8,700,BK,{letterSpacing:1})}}>FOR SALE</div>}
              </div>
              <div style={{..._row,gap:8,marginBottom:16}}>
                {selectedFeedItem.color&&<div style={{width:10,height:10,borderRadius:"50%",background:selectedFeedItem.color,border:"1px solid #FFFFFF22"}}/>}
                ...
                {selectedFeedItem.condition&&<div style={ss(10,400,DM)}>· {selectedFeedItem.condition}</div>}
              </div>
              <div style={{display:"flex",gap:10,marginBottom:20}}>
                {[[`$${selectedFeedItem.price||0}`,"PRICE"],[selectedFeedItem.category||"—","CATEGORY"],[selectedFeedItem.condition||"—","CONDITION"]].map(([v,l])=>(
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{handleWishlist(selectedFeedItem);setSelectedFeedItem(null);}} style={{flex:1,padding:"14px",borderRadius:16,background:"#1A1A2A",border:`1px solid ${G}44`,...ss(10,600,G,{letterSpacing:1}),cursor:_p}}>
                  ♡ WISHLIST
                </button>
                <button onClick={()=>{handleAddToCloset(selectedFeedItem);setSelectedFeedItem(null);}} style={{flex:1,padding:"14px",borderRadius:16,background:`linear-gradient(135deg,${G},#8A6E54)`,border:"none",...ss(10,700,BK,{letterSpacing:1.5}),cursor:_p}}>
                  + CLOSET
                </button>
              </div>
            </div>
