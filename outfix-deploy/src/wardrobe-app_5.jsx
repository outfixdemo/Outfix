                  ...
                  <button onClick={async()=>{
                    ...
                    try{
                      const raw = await callClaude(
                        `A user is describing a clothing item they want to add to their wardrobe app: "${voiceDesc}"\n\nGenerate 4 specific product matches they might be describing. For each, include a realistic product image URL from a major retailer (use real URLs from zara.com, cos.com, aritzia.com, net-a-porter.com, ssense.com, nordstrom.com or similar). Return ONLY JSON:\n{"results":[{"name":"...","brand":"...","category":"Tops|Bottoms|Dresses|Outerwear|Shoes|Accessories","color":"#hexcode","price":150,"emoji":"👚","condition":"New","tags":["..."],"imageUrl":"https://...actual-product-image-url..."},...]}`
                      );
                      const json=JSON.parse(raw.replace(/```json|```/g,"").trim());
                      setDescribeResults(json.results||[]);
