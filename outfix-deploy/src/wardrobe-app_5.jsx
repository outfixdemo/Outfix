  ...
  const loadLiveFeed = async () => {
    ...
    try {
      ...
      const [eventsData, profilesData] = await Promise.all([
        ...
        fetch(`${SB_URL}/rest/v1/profiles?id=in.(${ids})&select=id,username,avatar_url`, {
          headers: {...sbHeaders(session.access_token)}
        }).then(r=>r.json()),
      ]);
      const profileMap = {};
      ...
      if(Array.isArray(eventsData)){
        const mapped = eventsData.map(e=>({
          ...e,
          username: profileMap[e.user_id]?.username || "someone",
      ...
      <div style={{background:CD,borderRadius:20,overflow:"hidden",marginBottom:16,border:`1px solid ${G}22`}}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center",padding:"12px 16px"}}>
          <div style={{width:36,height:36,borderRadius:"50%",background:"#2A2A2A",overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>
            {event.avatarUrl ? <img src={event.avatarUrl} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/> : "👤"}
          </div>
          <div style={{flex:1}}>
            <div style={ss(11,600,MD,{letterSpacing:0.5})}>@{event.username}</div>
