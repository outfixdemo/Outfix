...
function HomeTab({items,outfits,showToast,setTab,setWishlist,addToWishlist,removeFromWishlist,setItems,session,onAddToCloset}){
  ...
  useEffect(()=>{
    ...
    const onMove = (e) => {
      ...
      if(delta > 8){
        ...
        setPullY(Math.min(80, delta));
      }
    };
    const onEnd = () => {
      if(pulling.current){
        setPullY(prev => {
          if(prev > 50){ loadLiveFeed(); showToast("Feed refreshed \u2746"); }
          return 0;
        });
        pulling.current = false;
      }
  ...
  return(
    <div ref={feedRef} style={{padding:"0 16px 24px"}}>
      ...
      {pullY > 0 && (
        <div style={{textAlign:"center",padding:"8px 0",marginBottom:4,transition:"opacity 0.2s",opacity:pullY/80}}>
          <div style={ss(9,400,DM,{letterSpacing:1})}>{pullY>50?"Release to refresh ✦":"Pull to refresh"}</div>
        </div>
      )}
